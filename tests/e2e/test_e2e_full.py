...
scripts/realmodel/test_e2e_http.py for those tests. Generation-triggering tests
...
class TestChatCompletion:
    ...
    See scripts/realmodel/test_e2e_http.py.
    """
...
class TestHealthVersion:
    ...
    def test_health_format(self):
        """Health response has the minimal public liveness payload.

        /health is unauthenticated (PUBLIC_PATHS), so it must NOT leak engine
        internals (model IDs, gpu_memory, queue depth). It returns only:
        status + engine.loaded + server_state + uptime_seconds. The rich detail
        is gated behind the authenticated /api/v1/monitoring/* router.
        """
        client = _client_with_engine()
        resp = client.get("/health")
        data = resp.json()
        ...
        assert "loaded" in data["engine"]
        assert "server_state" in data
        assert "uptime_seconds" in data
        # security: no internal state leaked to an anonymous caller
        assert "gpu_memory" not in data["engine"]
...
class TestResponseFormat:
    ...
    def test_health_response_format(self):
        """Health response has the minimal public liveness payload (no internal leak)."""
        client = _client_with_engine()
        resp = client.get("/health")
        assert resp.status_code == 200
        data = resp.json()
