...
def test_get_allowed_tokens_offers_eos_iff_can_terminate():
    ...
    assert 4242 in done.get_allowed_tokens(tok, []), (
        "top-level complete number must offer EOS"
    )
    assert 4242 not in nested.get_allowed_tokens(tok, []), (
        "nested number must NOT offer EOS"
    )
