...
class TestImageVariationsDenoise:
    """Regression : /images/variations carries NO prompt, so a high
    denoise_strength + empty prompt destroys the subject and the unguided denoise
    yields an unrelated image (verified live: a red-apple input produced a random
    human portrait at strength 0.8). The default must stay LOW so variations
    preserve the subject, and the handler must pass it to the engine."""
    ...
    def test_variations_handler_passes_denoise_to_engine(self):
        """The handler must forward denoise_strength into img_engine.generate."""
        import inspect
