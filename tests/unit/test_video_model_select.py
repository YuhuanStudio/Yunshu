...
def test_single_model_mismatch_not_404(monkeypatch):
    ...
    entries = [
        types.SimpleNamespace(model_id="my-custom-video", engine=only, is_loaded=True)
    ]
    ...
    resp = client.post(
        "/v1/video/generations", json={"prompt": "a cat"}
    )  # default model id
    os.environ.pop("YUNSHU_AUTH_DISABLED", None)
    assert resp.status_code != 404, (
        f"single loaded video model must not 404: {resp.text}"
    )
