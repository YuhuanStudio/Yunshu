...
def _client_with_image_engine(monkeypatch):
    ...
    monkeypatch.setattr(
        images_mod, "get_model_manager", lambda: _FakeManager(fake_engine)
    )
...
def test_malformed_control_image_returns_400(_client_with_image_engine):
    resp = _client_with_image_engine.post(
...
def test_source_guards_control_image_decode():
    ...
    src = (
        inspect.getsource(images.generate_image_endpoint)
        if hasattr(images, "generate_image_endpoint")
        else inspect.getsource(images)
    )
