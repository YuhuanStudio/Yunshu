"""tool-call streamer format-coverage + DeepSeek double-text fixes.

(HIGH): the DeepSeek ✿FUNCTION✿ transition pre-assigned self._buffer = buffer+token in
...
(HIGH): the streamer only understood XML <tool_call> + DeepSeek, so streaming a Mistral
...
def test_deepseek_transition_text_not_doubled():
...
def test_hermes_xml_unaffected():
