...
class TestBatchedEngineMTPRouting:
    ...
    def test_generate_spec_decode_falls_back_not_routes_to_mtp(self):
        """spec_decode=True now falls back to the verified fast
        path and does NOT route to the speculative routes (MTP/cross-model/n-gram).

        Real-model E2E proved the fast-path spec routes are not lossless (empty
        output on hybrid-recurrent caches, token duplication via the n-gram
        verifier) and give no speedup on bandwidth-bound Apple Silicon, so the
        ...
        engine._generate_fast = _fake_fast
        engine._mtp_decoder = object()  # truthy sentinel — should be bypassed
        ...
        try:
            ...
            assert not called["mtp"], "spec_decode must NOT route to MTP (falls back)"
            assert called["fast"], "spec_decode should fall back to the fast path"
    ...
    def test_mtp_routing_uses_mtp_decoder_not_strategy(self):
        """580: the dead _get_spec_strategy() was removed — MTP serving
