...
class TestCacheControlHints:
    def test_cache_control_in_text_block(self):
        ...
        hints = _extract_cache_control_hints(system)
        assert len(hints) == 1
        assert hints[0]["type"] == "ephemeral"

    def test_cache_control_none_system(self):
        """None system should return empty list."""
        assert _extract_cache_control_hints(None) == []

    def test_cache_control_string_system(self):
        """String system should return empty list."""
        assert _extract_cache_control_hints("You are helpful.") == []
