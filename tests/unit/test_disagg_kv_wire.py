...
defeating the entire split. serializes the KV blocks + first-token logits into the prefill
...
def test_prefill_wire_carries_kv_blocks():
    # a result WITH serialized KV blocks survives the prefill wire (the fix)
