...
class TestCancelEndpoint:
    ...
    def test_cancel_specific_request(self):
        ...
        result = asyncio.run(
            cancel_generation(
                CancelRequest(request_id="test-cancel-1"), self._make_mock_request()
            )
        )
    ...
    def test_cancel_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            asyncio.run(
                cancel_generation(
                    CancelRequest(request_id="nonexistent"), self._make_mock_request()
                )
            )
    ...
    def test_cancel_no_params(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            asyncio.run(cancel_generation(CancelRequest(), self._make_mock_request()))
