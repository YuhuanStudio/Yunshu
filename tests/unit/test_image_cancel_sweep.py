...
def test_engine_wrappers_thread_cancel_event():
    for name in (
        ...
        assert "self._run_cancellable(_make, cancel_event)" in src, (
            f"{name} not routed via _run_cancellable"
        )
...
def test_router_handlers_register_and_guard():
    for name in (
        ...
        assert "_register_image_cancel(req.model)" in src, (
            f"{name} no cancel registration"
        )
        assert "run_with_disconnect_guard(" in src, f"{name} no disconnect guard"
        assert "_unregister_image_cancel(_img_tracker, _img_id)" in src, (
            f"{name} no cleanup"
        )
