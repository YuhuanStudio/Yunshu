...
class TestRequestLifecycleOrchestrator:
    ...
    def test_pending_queue_promotion(self):
        ...
        assert orch.pending_count == 1
        assert orch.active_count == 0
        # when the scheduler DOES promote r2 (on_prefill_start), it must LEAVE the
        # pending queue. The old code left it there forever → _pending_queue grew unbounded
        # over every completed-while-over-limit request, eventually pinning at max_pending and
        # REJECTing+poisoning metrics. The pending id is now dropped on the QUEUED→PREFILLING
        # transition (and defensively at finish).
        orch.on_prefill_start("r2")
        assert orch.pending_count == 0
        assert orch.active_count == 1
        orch.on_request_finished("r2")
