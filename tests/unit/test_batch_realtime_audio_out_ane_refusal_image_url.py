...
def test_resample_converts_rate_streaming():
    ...
    pcm = struct.pack(
        f"<{n}h",
        *[int(8000 * math.sin(2 * math.pi * 440 * i / 44100)) for i in range(n)],
    )
    out = b""
...
def test_ane_refuses_non_mean(tmp_path):
    ...
    src = (
        inspect.getsource(ane_embedding.ANEEmbeddingProcessor.compile_model)
        if hasattr(ane_embedding.ANEEmbeddingProcessor, "compile_model")
        else inspect.getsource(ane_embedding)
    )
