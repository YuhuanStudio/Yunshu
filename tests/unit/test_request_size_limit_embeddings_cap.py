"""Waves 953 + 955: request size-limit byte enforcement + embeddings per-input length cap.

W953: the request-size middleware byte-counted the stream only for `transfer-encoding:
...
W955: the embeddings per-input length cap (_MAX_INPUT_TEXT_LENGTH) was checked only for a
...
def test_w953_byte_count_on_any_no_content_length():
...
def test_w955_list_str_length_cap_enforced():
    ...
    # a LIST containing an over-long string is now rejected too (W955)
    with pytest.raises(pydantic.ValidationError):
