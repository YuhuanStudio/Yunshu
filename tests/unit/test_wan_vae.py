...
def test_rmsnorm_l2_normalizes_channel():
    norm = WanRMSNorm(8)
    x = mx.array(
        np.random.default_rng(0).standard_normal((2, 3, 4, 4, 8)).astype(np.float32)
    )
    ...
    np.testing.assert_allclose(
        chan_norm, np.full((2, 3, 4, 4), 8**0.5), rtol=1e-3, atol=1e-3
    )
...
def test_causal_conv3d_shape_and_causal_pad():
    ...
    x = mx.array(
        np.random.default_rng(1).standard_normal((1, 5, 8, 8, 4)).astype(np.float32)
    )
    out = conv(x)
    mx.eval(out)
...
def test_causal_conv3d_cache_continuity():
    ...
    x = mx.array(
        np.random.default_rng(2).standard_normal((1, 3, 2, 2, 4)).astype(np.float32)
    )
    cache = mx.array(
        np.random.default_rng(3).standard_normal((1, 2, 2, 2, 4)).astype(np.float32)
    )
...
def test_residual_block_shape_in_eq_out():
    ...
    x = mx.array(
        np.random.default_rng(4).standard_normal((1, 3, 6, 6, 8)).astype(np.float32)
    )
    out = blk(x)
    mx.eval(out)
...
def test_residual_block_channel_change_uses_shortcut():
    ...
    x = mx.array(
        np.random.default_rng(5).standard_normal((1, 2, 4, 4, 8)).astype(np.float32)
    )
    out = blk(x)
    mx.eval(out)
...
def test_attention_block_shape_preserved():
    ...
    x = mx.array(
        np.random.default_rng(6).standard_normal((1, 2, 5, 5, 16)).astype(np.float32)
    )
    out = blk(x)
    mx.eval(out)
...
def test_avgdown3d_downsamples_t_and_channels():
    ...
    x = mx.array(
        np.random.default_rng(7).standard_normal((1, 4, 8, 8, 8)).astype(np.float32)
    )
    out = blk(x)
    mx.eval(out)
...
def test_dupup3d_upsamples_inverse_shape():
    ...
    x = mx.array(
        np.random.default_rng(8).standard_normal((1, 2, 4, 4, 16)).astype(np.float32)
    )
    out = blk(x)
    mx.eval(out)
...
def test_dupup3d_first_chunk_trims_temporal():
    ...
    x = mx.array(
        np.random.default_rng(9).standard_normal((1, 2, 3, 3, 16)).astype(np.float32)
    )
    out = blk(x, first_chunk=True)
    mx.eval(out)
    # T*2=4, trim (ft-1)=1 → 3
    assert out.shape[1] == 3
...
def test_numerical_parity_vs_torch_reference():
    ...
    m = np.asarray(
        WanAvgDown3D(8, 16, 2, 2)(mx.array(x.transpose(0, 2, 3, 4, 1)))
    ).transpose(0, 4, 1, 2, 3)
    ...
    m2 = np.asarray(
        WanDupUp3D(16, 8, 2, 2)(mx.array(xi.transpose(0, 2, 3, 4, 1)))
    ).transpose(0, 4, 1, 2, 3)
...
def test_resample_upsample_downsample_shape():
    ...
    x = mx.array(
        np.random.default_rng(11).standard_normal((1, 1, 8, 8, 8)).astype(np.float32)
    )
    out = up(x)
    mx.eval(out)
...
def test_decoder3d_full_decode_path_shape():
    ...
    dec = WanDecoder3d(
        dim=8, z_dim=4, dim_mult=(1, 2), num_res_blocks=1, temperal_upsample=(True,)
    )
    z = mx.array(
        np.random.default_rng(0).standard_normal((1, 1, 8, 8, 4)).astype(np.float32)
    )
    out = dec(z)
    mx.eval(out)
...
def test_real_wan_decoder_loads_and_decodes():
    ...
    z = mx.array(
        np.random.default_rng(0).standard_normal((1, 1, 16, 16, 48)).astype(np.float32)
    )
    out = dec(z)
    img = unpatchify_chw(out, 2)
    mx.eval(img)
...
def test_real_wan_encoder_loads_and_encodes():
    ...
    img = mx.array(
        np.random.default_rng(1).standard_normal((1, 1, 256, 256, 3)).astype(np.float32)
    )
...
def test_real_wan_vae_encode_decode_roundtrip():
    ...
    img = mx.array(
        np.random.default_rng(2).standard_normal((1, 1, 256, 256, 3)).astype(np.float32)
    )
...
def test_patchify_unpatchify_inverse():
    ...
    x = mx.array(
        np.random.default_rng(3).standard_normal((1, 1, 8, 8, 3)).astype(np.float32)
    )
...
def test_real_wan_vae_full_module():
    ...
    vae = load_wan_vae(_VAE)
    img = mx.array(
        np.random.default_rng(0).random((1, 1, 256, 256, 3)).astype(np.float32) * 2 - 1
    )
    z = vae.encode(img)
    recon = vae.decode(z)
    mx.eval(z, recon)
