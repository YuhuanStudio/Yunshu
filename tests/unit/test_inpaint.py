...
class TestMaskLoading:
    ...
    def _rgba_mask(self, transparent_rgb):
    ...
    def test_load_mask_rgba_transparent_white_rgb_is_kept(self):
        """an RGBA mask whose transparent region stores WHITE RGB underneath was
        wrongly inpainted by the old .convert('L') (alpha dropped). It must now be KEPT
        (transparent → black → keep), matching the documented white=inpaint convention."""
        from yunshu_engine.image_engine import ImageGenEngine
    ...
    def test_load_mask_rgba_transparent_black_rgb_still_kept(self):
        ...
        engine = ImageGenEngine.__new__(ImageGenEngine)
        mask = np.array(engine._load_mask(self._rgba_mask((0, 0, 0)), 16, 16))[0, 0]
        assert np.all(mask[:, :8] == 1.0)
        assert np.all(mask[:, 8:] == 0.0)

    def test_load_mask_opaque_grayscale_unchanged(self):
        # a plain grayscale mask (no alpha) must be byte-identical to before
        from yunshu_engine.image_engine import ImageGenEngine
