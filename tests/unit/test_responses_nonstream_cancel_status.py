...
(cross-tenant IDOR W714 holds across retrieve/cancel/chain; chaining order/roles/tool-items/
...
cancel_event.is_set() FIRST (W776) and reports "incomplete"; that fix was never propagated
...
def test_nonstream_status_checks_cancel_event_first():
    ...
    assert "_ns_cancel_event" in window and ".is_set()" in window, (
        ...
        "finish_reason map (W863)"
    )
    # and on a set cancel event it resolves to incomplete (mirrors the streaming W776 fix)
...
def test_streaming_path_still_has_cancel_first_check_parity():
    # parity guard: the streaming path's W776 cancel-first check must remain
