"""Waves 904-909: realtime protocol-completeness + middleware/admin hardening.

W904: realtime function_call output items are now announced via response.output_item.added
...
W906: prometheus_exporter._format_labels now escapes \\r (W876 keystone propagation).
W907: admin unload_model honors the bool return ("already_unloaded" when refused/no-op).
W908: realtime response.cancel with no active response emits an error (was a silent no-op).
W909: session.update validates modalities (list drawn from {text,audio}).
"""
...
def test_w904_function_call_emits_output_item_added_with_own_id():
...
def test_w905_metrics_middleware_is_outermost():
...
def test_w906_prometheus_exporter_escapes_carriage_return():
...
def test_w908_realtime_cancel_no_active_emits_error():
    from yunshu_gateway.routers import realtime
...
def test_w909_session_update_validates_modalities():
    ...
    for _name, obj in vars(_rt).items():
        if isinstance(obj, type) and hasattr(obj, "update") and hasattr(obj, "SUPPORTED_AUDIO_FORMATS"):
            cfg_cls = obj
            break
