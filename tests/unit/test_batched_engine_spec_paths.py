...
def _make_batched_engine(**overrides):
    ...
    engine._spec_decoder = overrides.get("_spec_decoder", None)
    engine._spec_enabled = overrides.get("_spec_enabled", False)
    engine._mtp_decoder = overrides.get("_mtp_decoder", None)
    engine._mtp_strategy = None
    engine._ngram_proposer = overrides.get("_ngram_proposer", None)
...
async def test_stream_generate_mtp_cancel_event():
    ...
    call_count = 0
    original_method = engine._stream_generate_mtp
