...
dir per model. W716 fixed cross-model disk-KV pollution for the default SSDKVCache
...
def _model_key(cfg: dict) -> str:
    """Mirror engine_core's W794 derivation: exact path if present, else an
    architecture signature."""
    key = str(cfg.get("_name_or_path", "") or cfg.get("name_or_path", ""))
    if not key:
        key = "-".join(str(cfg.get(a, "")) for a in
...
def test_distinct_models_get_distinct_dirs():
    base = "/tmp/ssd-w794"
    a = KVPrefixCache._scoped_ssd_dir(base, _model_key({"_name_or_path": "Qwen2.5-0.5B"}))
...
def test_arch_signature_fallback_distinguishes():
    ...
    qwen = _model_key({"model_type": "qwen2", "hidden_size": 896, "num_hidden_layers": 24, "vocab_size": 151936})
    llama = _model_key({"model_type": "llama", "hidden_size": 4096, "num_hidden_layers": 32, "vocab_size": 128256})
    assert qwen != llama
    assert KVPrefixCache._scoped_ssd_dir(base, qwen) != KVPrefixCache._scoped_ssd_dir(base, llama)


def test_empty_key_keeps_base_dir():
    # Back-compat: no derivable model identity → base dir unchanged.
    assert KVPrefixCache._scoped_ssd_dir("/tmp/ssd-w794", "") == "/tmp/ssd-w794"
