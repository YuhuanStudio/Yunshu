"""Waves 939-941: video fake-success, kv pressure-eviction abort, Responses function_call item.

W939 (HIGH): the native video pipeline stored the weights DICT in self._model (truthy but NOT
...
W940: evict_under_pressure called select_victim WITHOUT exclude=_skipped_indices, so a pinned
  victim was re-selected every iteration and the loop aborted ALL eviction on the first pin.
W941: response.output_item.added for a function_call omitted call_id/name/arguments.
"""
...
def test_w939_placeholder_tagged_fallback():
...
def test_w939_native_method_trips_503_guard():
...
def test_w939_stream_native_falls_back_on_non_callable():
...
def test_w940_evict_under_pressure_passes_exclude():
...
def test_w941_function_call_item_carries_fields():
    import json
...
def test_w1036_stream_function_call_added_uses_subscript_not_getattr():
    ...
    the EMPTY default on a dict, silently re-breaking W941 only on the stream path.
