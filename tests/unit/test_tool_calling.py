...
class TestExtractToolCallsFormats:
    ...
    def test_hermes_degenerate_json_string_no_crash(self):
        """<tool_call>"name"</tool_call> parses to a STR; the old
        `"name" in data` was a truthy substring match then data["name"] raised an
        UNCAUGHT TypeError (not JSONDecodeError) → 500. Must not crash."""
        text = '<tool_call>"name"</tool_call>'
        calls = extract_tool_calls(text)  # must not raise
        assert calls == []

    def test_hermes_json_array_of_calls(self):
        """a JSON array inside <tool_call> tags (parallel calls) was
        silently dropped by `"name" in data` on a list."""
        text = '<tool_call>[{"name": "fn1", "arguments": {}}, {"name": "fn2", "arguments": {"b": 2}}]</tool_call>'
        calls = extract_tool_calls(text)
        assert len(calls) == 2
        assert calls[0]["name"] == "fn1"
        assert calls[1]["name"] == "fn2"
    ...
    def test_llama_bare_json_parameters_not_dropped(self):
        """Llama 3.x emit bare-JSON {"name":..,"parameters":{..}} with no
        <tool_call> marker → the v2 fallback resolved the name but read only
        "arguments", silently dropping ALL args. Must accept the parameters alias."""
    ...
    def test_chatml_tool_calls_parameters_alias(self):
        ...
        text = '[TOOL_CALLS] [{"name": "g", "parameters": {"y": 2}}]'
        calls = extract_tool_calls_model_aware(text, "Mistral-7B-Instruct")
        assert len(calls) == 1
        assert json.loads(calls[0]["arguments"]) == {"y": 2}
...
class TestCleanToolCallMarkup:
    ...
    def test_removes_deepseek_block_markers(self):
        ...
        cleaned = clean_tool_call_markup(text)
        assert cleaned == "Sure."
        assert "tool" not in cleaned and "▁" not in cleaned and "Tokyo" not in cleaned

    def test_removes_glm_block_markers(self):
        """GLM <|tool_call_block_begin|>…<|tool_call_block_end|> leaked."""
        text = 'Done<|tool_call_block_begin|>foo\n```json\n{"x":1}\n```<|tool_call_block_end|>'
        cleaned = clean_tool_call_markup(text)
        assert cleaned == "Done"
        assert "tool_call_block" not in cleaned

    def test_removes_mistral_tool_calls_marker(self):
        """Mistral/Mixtral [TOOL_CALLS] [..] leaked into content (the cleaner
        had no rule for it; extraction was correct)."""
        text = 'Calling.[TOOL_CALLS] [{"name":"f","arguments":{"x":1}}]'
        cleaned = clean_tool_call_markup(text)
        assert cleaned == "Calling."
        assert "TOOL_CALLS" not in cleaned

    def test_mistral_marker_with_bracket_in_string(self):
        """Balanced strip must honor a `]` inside a JSON string argument."""
        text = 'Go [TOOL_CALLS] [{"name":"f","arguments":{"q":"a]b"}}] done'
        cleaned = clean_tool_call_markup(text)
        assert "TOOL_CALLS" not in cleaned and "a]b" not in cleaned
        assert "Go" in cleaned and "done" in cleaned
...
class TestToolCallStreamerBugfixes:
    """Tests for bugs found and fixed in the deep audit."""

    def test_flush_tag_end_parses_tool_call(self):
        """Bug: flush() in TAG_END state emitted JSON as text instead of parsing it.

        When the stream ends with a partial closing tag (</tool_call without >),
        the pending JSON body should be parsed as a tool call, not dumped as text.
        """
        streamer = ToolCallStreamer()
        tokens = [
            "<tool_call",
        ...
        all_outputs: list[StreamOutput] = []
        for t in tokens:
            outputs = streamer.process_token(t)
            all_outputs.extend(outputs)
        all_outputs.extend(streamer.flush())
    ...
    def test_closing_tag_inside_json_string_not_detected(self):
        """Bug: </tool_call inside a JSON string value was falsely detected as closing tag.

        The streamer now uses string-aware closing tag detection so that
        occurrences of '</tool_call' inside JSON string values are ignored.
        """
        streamer = ToolCallStreamer()
        # The arguments contain the literal string </tool_call as a value
        chunk = '<tool_call/>{"name": "echo", "arguments": {"text": "</tool_call"}}\n</tool_call/>'
        all_outputs: list[StreamOutput] = []
        for char in chunk:
            outputs = streamer.process_token(char)
            all_outputs.extend(outputs)
        ...
        tool_calls = [o for o in all_outputs if o.tool_call is not None]
        assert len(tool_calls) == 1
        assert tool_calls[0].tool_call.name == "echo"
        args = json.loads(tool_calls[0].tool_call.arguments)
        assert args["text"] == "</tool_call"

    def test_closing_tag_with_escaped_quote_not_detected(self):
        """Bug: escaped quotes before closing tag in string caused false detection.

        JSON string 'test\\\"</tool_call' should keep </tool_call inside the string.
        """
        streamer = ToolCallStreamer()
