...
class TestComputeRopeFreqs:
    ...
    def test_linear_scaling_divides_by_factor(self):
        ...
        for b, s in zip(base, scaled, strict=False):
    ...
    def test_yarn_uses_plain_base_not_ntk_base(self):
        """YaRN freq_extra must use plain base_freq, not NTK-adjusted base.

        This verifies the fix for a bug where freq_extra was computed from
        ntk_base (base * factor^(d/(d-2))) instead of plain base_freq.
        The MLX-LM reference YarnRoPE uses plain base for freq_extra.
        """
        base = 10000.0
        head_dim = 64
        head_dim // 2
        ...
        # For i=15 (out of 32 half-dim), the NTK base gives different values
        correct_extra_15 = base ** (2 * 15 / head_dim)
        ntk_wrong_15 = ntk_base_wrong ** (2 * 15 / head_dim)

        # They should be different (NTK base is larger)
        if abs(correct_extra_15 - ntk_wrong_15) > 0.001:
            # The YARN output for dimension 15 should NOT match the NTK-wrong value
            # It should be closer to the correct plain-base behavior
            # This is a regression test for the NTK-base bug
            assert yarn_freqs[15] != ntk_wrong_15 * scaling_factor
...
class FakeKVPrefixCache:
    ...
    def get(self, tokens, exact_refeed_trim: bool = True):
