...
def test_inprogress_batch_not_evicted_by_ttl():
    ...
    B._batch_store["running"] = {"id": "running", "status": "in_progress", "started_at": old}
...
def test_size_eviction_skips_inprogress():
    ...
    try:
        ...
        B._batch_store["r1"] = {"id": "r1", "status": "in_progress", "started_at": now - 5}
        B._batch_store["r2"] = {"id": "r2", "status": "in_progress", "started_at": now - 4}
        B._batch_store["d1"] = {"id": "d1", "status": "completed", "started_at": now - 3}
        B._cleanup_batch_store()
...
def test_progress_write_is_guarded():
    src = inspect.getsource(B.create_batch)
    assert "_batch_store.get(batch_id)" in src  # both progress + final writes use .get()
