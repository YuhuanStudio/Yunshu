...
def test_mxfp4_quantization_config_derived():
    ...
    assert _derive_vlm_quantization(cfg) == {"group_size": 32, "bits": 4, "mode": "mxfp4"}


def test_compressed_tensors_derived():
    cfg = {"quantization_config": {"quant_method": "compressed-tensors"}}
    assert _derive_vlm_quantization(cfg) == {"group_size": 32, "bits": 4, "mode": "affine"}
...
def test_unsupported_method_returns_none():
    for m in ("awq", "gptq", "bitnet"):
        assert _derive_vlm_quantization({"quantization_config": {"quant_method": m}}) is None
