...
class TestOutputTokenTracking:
    """Verify output_token_ids are properly tracked ()."""
...
class TestGatewayIntegration:
    ...
    def test_health_with_stats(self):
        ...
        # SECURITY: public /health exposes only engine-loaded (a liveness bool),
        # NOT the full engine.get_stats() (step_counter/num_requests_processed/model id —
        # reconnaissance). The detailed stats are behind authenticated /api/v1/monitoring.
        assert data["engine"]["loaded"] is True
        assert "step_counter" not in data["engine"]
        assert "num_requests_processed" not in data["engine"]
