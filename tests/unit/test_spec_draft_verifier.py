...
class TestVerify:
    ...
    def test_trim_decrements_offset_without_resizing_buffer(self):
        """_trim_cache decrements offset and does NOT resize the buffer.

        Earlier code manually sliced ``c.keys``/``c.values`` to the new offset
        after trimming, on the theory that stale K/V beyond the offset would
        corrupt the verify forward.  In practice that manual resize BROKE
        mlx-lm KVCache.update_and_fetch's reallocation bookkeeping on the next
        forward — the offset stopped decrementing and rejected drafts
        accumulated, producing the degenerate ".txt.txt…" output.  mlx-lm's
        KVCache deliberately keeps spare capacity beyond ``offset`` and bounds
        every read by ``offset``, so stale data is never attended to; its own
        speculative_generate_step trims with offset-decrement alone.  The fix
        trusts that: trim the offset, leave the buffer's capacity intact.
        """

        class TrimAwareCache:
            """Mimics mlx-lm KVCache layout (B, H, T, D) with offset."""

            def __init__(self, T: int = 10):
                self.keys = mx.zeros((1, 4, T, 8))
                self.values = mx.zeros((1, 4, T, 8))
                self.offset = T
        ...
        cache_obj = TrimAwareCache(T=10)
        assert cache_obj.keys.shape[-2] == 10
        assert cache_obj.offset == 10

        n_trimmed = _trim_cache([cache_obj], 3)

        # Offset is decremented (the logical trim) ...
        assert n_trimmed == 3
        assert cache_obj.offset == 7
        # ... but the buffer capacity is LEFT INTACT (no manual slice). mlx-lm
        # bounds reads by offset, so the spare capacity is harmless and resizing
        # it would corrupt the next update_and_fetch.
        assert cache_obj.keys.shape[-2] == 10
        assert cache_obj.values.shape[-2] == 10

    def test_trim_slice_safe_when_no_keys(self):
        """_trim_cache must not raise on caches lacking keys/values attrs."""
        result = _trim_cache([MockCache(20)], 5)
        # MockCache.trim returns the trimmed count; slice is best-effort skip.
        assert result == 5
...
class TestVerifyWithLastToken:
    ...
    def test_partial_match(self):
        ...
        assert result.rejection_position == 1
        assert result.rejected_count == 2
        # Cache: trimmed rejected_count entries (rollback + forward leaves K new)
        assert result.cache_trimmed == 2
    ...
    def test_total_mismatch(self):
        ...
        assert result.cache_trimmed == 3  # rejected_count entries (total mismatch)
    ...
    def test_single_draft_rejected(self):
        ...
        assert result.bonus_token == 99
        assert result.rejection_position == 0
        assert result.cache_trimmed == 1  # rejected_count = 1
    ...
    def test_rollback_and_rejection_trimming(self):
        ...
        assert result.accepted_count == 1
        assert result.cache_trimmed == 2  # rejected_count = 2
...
# ── Sampler acceptance uses argmax (not stochastic) tests ──


class TestSamplerAcceptanceArgmax:
    """Tests verifying that acceptance comparison uses deterministic argmax
    even when a sampler is provided, while bonus token uses the sampler.

    Bug: Previously, sampler output was used for BOTH acceptance comparison
    AND bonus selection. This meant stochastic samplers could randomly reject
    tokens the target model actually preferred, producing incorrect output
    distributions. The fix: acceptance always uses argmax; sampler only
    affects the bonus/correction token.
    """

    def setup_method(self):
        self.verifier = SpecDraftVerifier(track_stats=True)

    def test_verify_acceptance_uses_argmax_not_sampler(self):
        """verify(): acceptance uses argmax even when sampler would disagree.

        MockModel picks [10, 15, 20] (argmax). draft_ids = [5, 10, 15].
        Shifted comparison: argmax[0]=10 vs draft[1]=10 -> match.
        argmax[1]=15 vs draft[2]=15 -> match. All accepted.

        A "broken" sampler that always picks 99 should NOT affect acceptance.
        """
        model = MockModel([10, 15, 20])
        cache = [MockCache(10)]

        def broken_sampler(logits):
    ...
    def test_verify_with_last_token_acceptance_uses_argmax(self):
        """verify_with_last_token(): acceptance uses argmax, not sampler.

        MockModel picks [5, 10, 15, 99]. draft_ids = [5, 10, 15].
        Aligned comparison: argmax[0]=5 vs draft[0]=5 -> match.
        argmax[1]=10 vs draft[1]=10 -> match. argmax[2]=15 vs draft[2]=15 -> match.
        All accepted, bonus from position 3 = 99.
        """
        model = MockModel([5, 10, 15, 99])
        cache = [MockCache(20)]
    ...
    def test_verify_bonus_uses_sampler(self):
        """verify(): bonus token uses the sampler for temperature-based output.

        model picks [10, 99, 20], draft_ids = [5, 10, 15].
        argmax[0]=10 vs draft[1]=10 -> match.
        argmax[1]=99 vs draft[2]=15 -> mismatch at position 2.
        bonus from position 1 (rejection_position - 1 = 1).
        sampler should be used for bonus, not argmax.
        """
        model = MockModel([10, 99, 20])
        cache = [MockCache(10)]

        def force_42_sampler(logits):
            return mx.array([[42]])

        result = self.verifier.verify(
            model, [5, 10, 15], cache, sampler=force_42_sampler
        )

        # Acceptance uses argmax: d0 trusted, d1 verified, d2 rejected
        assert result.accepted_count == 2
        assert result.accepted_tokens == [5, 10]
        # Bonus uses sampler: 42 (not argmax[1]=99)
        assert result.bonus_token == 42
