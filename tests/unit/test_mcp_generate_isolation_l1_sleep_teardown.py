"""Waves 956 + 957: MCP generate model-isolation regardless of args shape + L1 sleep teardown.

W956: the MCP tools/call `generate` model-isolation gate fired only when arguments was a
...
W957: L1 sleep leaked the entire engine subsystem — it manually nulled
...
def test_w956_mcp_generate_gate_regardless_of_args_shape():
...
def test_w957_l1_sleep_stops_engine_and_frees_manager():
    from yunshu_gateway.routers import sleep
    src = inspect.getsource(sleep)
    code = "\n".join(ln.split("#", 1)[0] for ln in src.splitlines())
    # contextlib is imported (the W957 edit uses contextlib.suppress)
...
def test_w957_engine_stop_precedes_null():
...
def test_w958_repetition_penalty_zero_is_noop_not_nan():
    """W958: repetition_penalty=0.0 (allowed by the gateway ge=0.0 schemas) hit the
