...
class TestMTPLoad:
    ...
    def test_load_creates_mtp_module(self, model_name):
    ...
    def test_mtp_module_has_expected_layers(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
    ...
    def test_mtp_fc_weight_shape(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
    ...
    def test_make_mtp_cache(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
...
class TestMTPForward:
    ...
    def test_return_hidden_shape(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
    ...
    def test_mtp_forward_logits_shape(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
        ...
        mtp_logits = model.mtp_forward(hidden, next_tok, mtp_cache)

        vocab_size = inner.args.vocab_size
        assert mtp_logits.shape == (1, 1, vocab_size)

    def test_mtp_forward_with_multi_position_hidden(self, model_name):
    ...
    def test_mtp_prediction_is_valid_token(self, model_name):
        from yunshu_engine.mtp_patch import load_model_with_mtp
...
class TestMTPDecodeLoop:
    ...
    def test_mtp_acceptance_reasonable(self, model_name):
        ...
        for _ in range(total):
            ...
            mtp_cache = inner.make_mtp_cache()
            mtp_logits = model.mtp_forward(last_hidden, mx.array([[current_tok]]), mtp_cache)
            mtp_tok = int(mx.argmax(mtp_logits[0, -1, :]).item())

            t_logits = model(mx.array([[current_tok]]), cache=cache)
            ...
            _, hidden = model(mx.array([[current_tok]]), cache=cache, return_hidden=True)
