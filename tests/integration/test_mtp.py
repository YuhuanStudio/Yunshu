...
def _has_model(name: str) -> bool:
    """Return True iff model dir exists AND its safetensors blobs are non-empty.

    HuggingFace cache symlinks can survive a partial `huggingface_hub` purge —
    the model dir + index file remain, but the underlying blobs are 0 bytes.
    A subsequent `mlx_lm.load()` hangs indefinitely trying to mmap them.
    """
    d = MODELS_DIR / name
    if not d.exists():
        return False
    shards = list(d.glob("model-*.safetensors"))
    if shards and any(s.stat().st_size == 0 for s in shards):
        return False
    single = d / "model.safetensors"
    return not (single.exists() and single.stat().st_size == 0)


def _has_mtp_weights(name: str) -> bool:
    p = MODELS_DIR / name / "mtp-weights.safetensors"
