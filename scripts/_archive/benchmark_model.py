...
def main():
    ...
    # Handle case where MODELS_DIR is a symlink (possibly dangling, e.g. external drive unmounted)
    if MODELS_DIR.is_symlink():
        target = MODELS_DIR.resolve()
        if not target.exists():
            print(f"  [ERROR] {MODELS_DIR} is a symlink to {target!s} which does not exist.")
            print("  [ERROR] Mount the external drive or remove the symlink before retrying.")
            sys.exit(1)
    else:
