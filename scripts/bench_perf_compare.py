...
def bench_mlxlm():
    ...
    for label, prompt, max_tok in [
        ...
        for _ in range(3):
            ...
            first_tok_time = None
            n_tok = 0
            for resp in stream_generate(model, tokenizer, prompt, max_tokens=max_tok, sampler=sampler):
        ...
        avg_tps = sum(t / s for t, s in zip(total_toks, total_times)) / len(total_toks)
        results[label] = {
