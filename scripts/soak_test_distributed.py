...
class SoakTestConfig:
    ...
    ops: list[str] = field(default_factory=lambda: ["all_reduce", "all_gather", "send_recv"])
...
class SoakTestRunner:
    ...
    def run_single_op(self, op_name: str, group: Optional[object] = None) -> dict:
    ...
    def run_iteration(self, group: Optional[object] = None) -> dict:
    ...
    def run(self, group: Optional[object] = None) -> SoakTestReport:
        ...
        while (time.monotonic() - self._start_time) < duration_s:
            try:
    ...
    def _build_report(self, elapsed_hours: float) -> SoakTestReport:
        ...
        for op_name in self._config.ops:
            op_lats = [lat for lat in self._latencies]  # all lats mixed; we track globally
            op_results[op_name] = {
