r"""Yunshu Comprehensive Benchmark — lm-evaluation-harness compatible.
...
async def run_mmlu_pro_vllm_mlx(
    ...
    P("    MMLU-Pro (vllm-mlx): loading model...")
...
async def main_async(args):
    ...
    P(f"{'═' * 90}")
    P("  Yunshu Benchmark — lm-evaluation-harness compatible")
    P(f"{'═' * 90}")
    P("  Extraction: answer is \\(?([A-J])\\)?")
    P("  Method: 5-shot CoT (lm-eval-harness) + chat_template")
    P("  Max tokens: 4096 | Temperature: 0.0 (greedy)")
    P(f"  Batch size: {batch_size}")
    if samples == 0:
        P("  Samples: FULL dataset (12,032)")
    else:
        P(f"  Samples: {samples}")
    ...
    if "yunshu" in frameworks and "mmlu_pro" in benchmarks:
        ...
        if cached:
            ...
            P(f"\n{'─' * 90}")
            P("  Framework: Yunshu (BatchedEngine)")
            P(f"{'─' * 90}")
            P("\n  MMLU-PRO (5-shot CoT + chat_template):")
    ...
    if "vllm-mlx" in frameworks and "mmlu_pro" in benchmarks:
        ...
        if cached:
            ...
            P(f"\n{'─' * 90}")
            P("  Framework: vllm-mlx (SimpleEngine)")
            P(f"{'─' * 90}")
            P("\n  MMLU-PRO (5-shot CoT + chat_template):")
    ...
    if "omlx" in frameworks and "mmlu_pro" in benchmarks:
        ...
        if cached:
            ...
            P(f"\n{'─' * 90}")
            P("  Framework: omlx (BatchedEngine)")
            P(f"{'─' * 90}")
            P("\n  MMLU-PRO (5-shot CoT + chat_template):")
    ...
    if all_results:
        ...
        # all_results is only populated by mmlu_pro; perf and multimodal use
        # their own data structures (perf_data, mm_results) and report inline.
        # Only emit "no results" when mmlu_pro was requested but produced nothing.
        P("\nNo MMLU-Pro results collected.")
