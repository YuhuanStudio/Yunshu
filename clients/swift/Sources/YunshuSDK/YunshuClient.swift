...
///
/// `completionTokens` is optional: the `/v1/embeddings` endpoint returns a
/// `usage` object with only `prompt_tokens` and `total_tokens` (no
/// `completion_tokens`), so a non-optional field fails to decode a real
/// embeddings response. Chat/completions always include all three.
...
public final class YunshuClient: Sendable {
    ...
    ///   - model:       Model ID. Required by the server — a request without a
    ///                  model is rejected with HTTP 400.
