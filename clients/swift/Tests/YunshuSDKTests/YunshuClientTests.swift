...
struct YunshuClientTests {
    ...
    func decodeEmbeddingResponse() throws {
        ...
        // Note: the real /v1/embeddings server response omits `completion_tokens`
        // (only prompt_tokens + total_tokens). This fixture matches that exactly
        // so the test would catch a regression to a non-optional completionTokens.
        ...
        #expect(resp.usage?.completionTokens == nil)
    }
