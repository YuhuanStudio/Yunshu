        ...
        for key, value in data.items():
            if hasattr(self, key):
                ...
                if key == "tool_choice":
                    ...
                    if not (value in ("auto", "none", "required") or isinstance(value, dict)):
                        continue
...
class Conversation:
    ...
    def add_item(self, item: ConversationItem, previous_item_id: str | None = None) -> None:
...
class RealtimeSession:
    ...
    def _build_encode_lut(cls, decode_table: list[int]) -> bytes:
        ...
        for pcm in range(-32768, 32768):
            ...
            if j <= 0:
                ...
                best = codes[j] if abs(vals[j] - pcm) < abs(vals[j - 1] - pcm) else codes[j - 1]
            lut[pcm & 0xFFFF] = best
        return bytes(lut)
    ...
    def _resample_pcm16_linear(self, pcm16: bytes, from_rate: int, to_rate: int,
                               state_attr: str) -> bytes:
    ...
    def _encode_output_audio(self, pcm16: bytes, in_rate: int = 24000,
                             fmt: str | None = None) -> tuple[bytes, int]:
        ...
        if fmt == "g711_ulaw" or fmt == "g711_alaw":
            if in_rate == 24000:
                ...
                pcm8k = self._resample_pcm16_linear(pcm16, in_rate, 8000, "_g711_lin_state")
            n = len(pcm8k) // 2
            import struct
            samples = struct.unpack(f"<{n}h", pcm8k[: n * 2]) if n else ()
            enc = self._get_ulaw_encode_lut() if fmt == "g711_ulaw" else self._get_alaw_encode_lut()
        ...
        pcm24 = pcm16 if in_rate == 24000 else self._resample_pcm16_linear(
            pcm16, in_rate, 24000, "_pcm16_lin_state")
    ...
    async def _handle_conversation_item_create(self, event: dict) -> None:
        ...
        if self.conversation.has_item(item_id):
            await self.send_event(_event(
                RealtimeEvent.ERROR,
                error={"message": f"Item id '{item_id}' already exists",
        ...
        if _csize > _max_input_audio_bytes():
            await self.send_event(_event(
                RealtimeEvent.ERROR,
                error={"message": "Item content exceeds the maximum allowed size",
                       "type": "invalid_request_error"},
            ))
            return
    ...
    async def _handle_conversation_item_truncate(self, event: dict) -> None:
        ...
        if not item_id:
            ...
            return
        target = next((it for it in self.conversation.items if it.item_id == item_id), None)
        if target is None:
            await self.send_event(_event(
                RealtimeEvent.ERROR,
                error={"message": f"Item {item_id} not found", "type": "invalid_request_error"},
        ...
        try:
            ...
            for c in (target.content or []):
                if isinstance(c, dict) and c.get("type") in ("audio", "input_audio"):
                    ...
                    if _tr and isinstance(_dur, (int, float)) and _dur > 0 and audio_end_ms < _dur:
    ...
    async def _generate_response(
        ...
        try:
            ...
            max_tokens = _coerce_max_response_tokens(
                config.get("max_response_output_tokens", self.session.max_response_output_tokens),
            ...
            _snap_tool_choice = config.get("tool_choice", getattr(self.session, "tool_choice", "auto"))
            ...
            if tool_calls:
                for tc in tool_calls:
                    _fc_item = ConversationItem(
                        ...
                        call_id=f"call_{uuid.uuid4().hex[:8]}", name=tc.name,
            ...
            if "audio" in modalities and _visible_text:
                await self._synthesize_audio_response(_visible_text, response_id, item_id, voice=_snap_voice, out_fmt=_snap_out_fmt)
            ...
            for _fc_i, _fc_item in enumerate(_fc_items):
                ...
                await self.send_event(_event(
                    ...
                    item={"id": _fc_item.item_id, "object": "realtime.item",
                          "type": "function_call", "status": "in_progress",
                          "name": _fc_item.name, "call_id": _fc_item.call_id,
                          "arguments": ""},
                ))
                await self.send_event(_event(
                    RealtimeEvent.RESPONSE_FUNCTION_CALL_ARGUMENTS_DELTA,
                    response_id=response_id, item_id=_fc_item.item_id,
                    output_index=_fc_oidx, call_id=_fc_item.call_id,
                    name=_fc_item.name, delta=_fc_item.arguments,
                ))
                await self.send_event(_event(
                    RealtimeEvent.RESPONSE_FUNCTION_CALL_ARGUMENTS_DONE,
                    response_id=response_id, item_id=_fc_item.item_id,
                    output_index=_fc_oidx, call_id=_fc_item.call_id,
                    name=_fc_item.name, arguments=_fc_item.arguments,
                ))
                await self.send_event(_event(
                    "response.output_item.done",
                    response_id=response_id, output_index=_fc_oidx,
            ...
            await self.send_event(_event(
                ...
                response={
                    ...
                    "object": "realtime.response",
                    "status": "completed",
                    "output": [assistant_item.to_dict()] + [fi.to_dict() for fi in _fc_items],
                    "usage": {
    ...
    async def _handle_response_cancel(self, event: dict) -> None:
        ...
        if task and not task.done():
            ...
            if _had_audio:
                await self._locked_send_event(_event(
                    RealtimeEvent.RESPONSE_AUDIO_DONE,
                    response_id=response_id,
                    item_id=item_id,
                    output_index=0,
            ...
            await self._close_response_item(response_id, item_id, status="incomplete", locked=True)
            await self._locked_send_event(_event(
                RealtimeEvent.RESPONSE_DONE,
                response={
                    "id": response_id,
    ...
    async def _close_response_item(self, response_id: str, item_id: str,
                                   status: str = "incomplete", text: str = "",
        ...
        await _send(_event(
            ...
            response_id=response_id, output_index=0,
            item={"id": item_id, "type": "message", "role": "assistant",
                  "content": [{"type": "text", "text": text}], "status": status},
        ))

    async def _synthesize_audio_response(
        self, text: str, response_id: str, item_id: str, voice: str | None = None,
            ...
            for entry in manager.list_entries():
                if entry.is_loaded and hasattr(entry.engine, 'synthesize'):
                    ...
                    if hasattr(engine, 'synthesize_stream'):
                        ...
                        _in_rate = int(_raw_rate) if isinstance(_raw_rate, (int, float)) and _raw_rate > 0 else 24000
                        self._pcm16_lin_state = None
                        self._g711_lin_state = None
                        async for chunk in engine.synthesize_stream(
                            ...
                            audio, _csz = self._encode_output_audio(audio, _in_rate, fmt=out_fmt)
                        ...
                        if audio_data is not None:
                            ...
                            raw_audio = _strip_wav_header(raw_audio)  # full WAV → raw PCM16
                            raw_audio, _csz = self._encode_output_audio(raw_audio, _in_rate, fmt=out_fmt)  # → output fmt
                            offset = 0
        ...
        if speech:
            ...
            if self._vad_speaking:
                ...
                if self._vad_silence_bytes / _bytes_per_ms >= silence_duration_ms:
                    ...
                    _speech_end = max(0, _pre_offset + len(audio_chunk) - self._vad_silence_bytes)
                    self._vad_silence_bytes = 0
                    await self.send_event(_event(
    ...
    async def _handle_input_audio_buffer_commit(self, event: dict, *, vad_trim: bool = False) -> bool:
                ...
                for entry in manager.list_entries():
                    if entry.is_loaded and entry.engine is not None:
                        if getattr(entry, 'model_id', None) == self.session.model:
                            return entry.engine if _allowed(self.session.model) else None
            # Fall back to first loaded engine
            for entry in manager.list_entries():
                if entry.is_loaded and entry.engine is not None:
                    return entry.engine if _allowed(getattr(entry, 'model_id', None)) else None
...
async def realtime_endpoint(ws: WebSocket):
    ...
    # been removed.
    auth_disabled = os.environ.get("YUNSHU_AUTH_DISABLED", "").lower() in ("true", "1", "yes")
