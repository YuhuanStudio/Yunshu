...
Gateway monitoring endpoints: system stats, model status, active requests —
for real-time observability by operators and Prometheus scraping.
...
async def engine_stats(request: Request) -> dict[str, Any]:
    """Engine-level statistics — gateway view aggregating loaded engines.

    Mirrors /api/v1/monitoring/engine (L2 control plane) so the /gw/
    monitoring tree is feature-complete; some clients (WebUI, monitors)
    historically hit /api/v1/gw/monitoring/* exclusively and the
    previously-missing /engine path returned 404.
    """
    _check_permission(request)
    engines = _collect_engines_from_globals()
    if not engines:
        return {"loaded": False, "model": None, "engines": []}
    per_engine = []
    total_active = 0
    total_waiting = 0
    total_processed = 0
    ...
    for model_id, engine in engines:
        ...
        try:
            ...
            logger.debug("engine.get_stats failed for %s", model_id, exc_info=True)
            continue
        per_engine.append({"model_id": model_id, "stats": s})
        total_active += s.get(
            "active_collectors",
            s.get("scheduler_running") or s.get("active") or 0,
        )
        total_waiting += s.get("scheduler_waiting") or s.get("waiting") or 0
        total_processed += s.get("num_requests_processed", 0)
        step_counter = max(
            step_counter, s.get("scheduler_step_counter", s.get("step_counter", 0))
        )
    # ServerMetrics is the source of truth for total counters (the fast
    # path bypasses scheduler counters).
    ...
    try:
        ...
        if total_processed == 0:
            total_processed = int(snap.get("total_requests", 0))
    except Exception:
        logger.debug("ServerMetrics snapshot unavailable", exc_info=True)
    return {
        "loaded": True,
        "model": ", ".join(m for m, _ in engines),
        "engines": per_engine,
...
async def spec_decode_stats(request: Request) -> dict[str, Any]:
    ...
    if manager is not None:
        ...
        engine = get_engine()
        if engine and isinstance(engine, BatchedEngine):
            results.append(_spec_info(engine.model_name, engine))
...
async def radix_tree_stats(request: Request) -> dict[str, Any]:
    ...
    if manager is not None:
        ...
        # Single-model mode: no manager; fall back to the lone engine so the
        # endpoint reports real state instead of an empty list (which reads as
        # "no models loaded").
        engine = get_engine()
        if engine and isinstance(engine, BatchedEngine):
            try:
...
async def inflight_prefix_sharing_stats(request: Request) -> dict[str, Any]:
    """Inflight prefix sharing statistics.
...
async def all_monitoring_stats(request: Request) -> dict[str, Any]:
    ...
    """
    _check_permission(request)
    # Mark request so sub-handlers skip redundant auth checks.
    request.state._monitoring_authed = True
    result: dict[str, Any] = {}
