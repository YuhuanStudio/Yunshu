...
class BaseBenchmark(ABC):
    ...
    def _extract_mc_answer(response: str, valid_letters: list[str]) -> str:
        ...
        matches = re.findall(
            r"(?:answer\s*(?:is|:)\s*)([" + pattern + r"])\b", upper
        )
        if matches:
            return matches[-1]
...
class GSM8KBenchmark(BaseBenchmark):
    ...
    def format_prompt(self, item: dict) -> list[dict[str, str]]:
        return [{"role": "user", "content": f"Solve this step by step:\n\n{item['question']}"}]
...
class HellaSwagBenchmark(BaseBenchmark):
    ...
    def check_answer(self, predicted: str, item: dict) -> bool:
        return predicted.upper() == str(item.get("label", item.get("answer", ""))).upper()
    ...
    def check_answer(self, predicted: str, item: dict) -> bool:
        ...
        for ans in correct_answers:
            if ans.lower().strip() in predicted_lower:
                return True
        if best_answer and best_answer.lower().strip() in predicted_lower:
            return True
        return False
...
class HumanEvalBenchmark(BaseBenchmark):
    ...
    def format_prompt(self, item: dict) -> list[dict[str, str]]:
        ...
        return [{"role": "user", "content": f"Complete the following Python function. Write ONLY the function body, no explanation:\n\n```python\n{prompt}```"}]
...
class PIQABenchmark(BaseBenchmark):
    ...
    def format_prompt(self, item: dict) -> list[dict[str, str]]:
        ...
        sol1 = item.get("sol1", "")
        sol2 = item.get("sol2", "")
        return [{"role": "user", "content": f"Which solution is more sensible?\n\nGoal: {goal}\n\nA. {sol1}\nB. {sol2}\n\nAnswer A or B."}]
    ...
    def check_answer(self, predicted: str, item: dict) -> bool:
        correct = str(item.get("label", item.get("answer", "")))
...
def _load_jsonl(filename: str, sample_size: int = 0) -> list[dict]:
    ...
    if not path.exists():
        ...
        console.print("[dim]Datasets should be placed in python/yunshu_cli/eval_data/[/]")
        return []
...
def run_eval(
    ...
    model: Optional[str] = typer.Option(None, "--model", "-m", help="Model to evaluate."),
    sample: int = typer.Option(0, "--sample", "-n", help="Sample size (0 = full dataset)."),
    quick: bool = typer.Option(False, "--quick", "-q", help="Quick run with 50 samples."),
):
    ...
    console.print(f"[bold]Running[/] {bench.name} ({len(items)} questions, model={model})")
