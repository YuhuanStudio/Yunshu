...
class BlockTable:
    ...
    def append_blocks(self, blocks: list[KVBlock]) -> None:
        ...
        The previous last block (if any) is left as-is — its occupancy is
        NOT promoted to block_size.  Callers must ensure the previous last
        block is full before calling this method, or correct total_tokens
        afterward (same pattern as append_block).
        """
        if not blocks:
            return
    ...
    def trim_prefix_blocks(self, n: int) -> list[KVBlock]:
        """Remove and return the first *n* blocks from the table.

        Used by sliding-window attention to evict blocks that have fallen
        outside the window.  The caller is responsible for freeing the
        returned blocks via ``BlockPool.free()``.

        Args:
            n: Number of prefix blocks to trim (must be <= num_blocks).

        Returns:
            List of removed KVBlock objects (in logical order).

        Raises:
            ValueError: If n is negative or exceeds the table size.
        """
        if n < 0:
            raise ValueError(f"trim_prefix_blocks: n must be >= 0, got {n}")
        if n == 0:
            return []
        if n > len(self._blocks):
            raise ValueError(
                f"trim_prefix_blocks: cannot trim {n} blocks, "
                f"table only has {len(self._blocks)}"
            )
        trimmed = self._blocks[:n]
        self._blocks = self._blocks[n:]
        # Recompute total_tokens from scratch.  All remaining full blocks
        # contribute block_size tokens; the last block contributes
        # _last_block_occupancy (which is unchanged because we only
        # removed prefix blocks).
        if not self._blocks:
            # All blocks trimmed — reset everything to zero to avoid
            # reporting stale occupancy for a now-empty table.
            self.total_tokens = 0
            self._last_block_occupancy = 0
        else:
