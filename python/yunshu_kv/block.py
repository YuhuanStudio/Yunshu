...
class KVBlock:
    ...
    is_null: bool = False
    prev: KVBlock | None = None
    next: KVBlock | None = None
...
class BlockPool:
    ...
    def __init__(
        ...
        # Using RLock so cow_block_in_table can hold the lock and call cow_block
    ...
    def lookup_and_free(self, block_hash: int) -> bool:
        """Atomically look up a cached block by hash and free it.

        Combines lookup_hash + evict_and_free into a single lock acquisition,
        preventing TOCTOU races where a concurrent allocate() could claim
        the block between the separate lookup and free lock scopes.
