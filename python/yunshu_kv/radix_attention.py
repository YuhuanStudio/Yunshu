...
Improves on flat block-level prefix matching with a radix tree
that supports:
...
class RadixNode:
    ...
    def path_blocks(self) -> list[KVBlock]:
        """Collect all blocks from root to this node, in correct token order."""
        # Walk self→root collecting each node's block LIST, then
        # reverse the NODE order — NOT a flat result.reverse(), which also reverses
        # the blocks WITHIN each node. With the real block_size=64 and prompts >64
        # tokens a matched node holds many blocks, so a flat-reverse returns
        # every multi-block radix prefix hit with its KV blocks SCRAMBLED (logical
        # block 0 → the physical block holding the prefix's LAST tokens) → wrong
        # attention context → wrong output. (Tests missed it: they used block_size=1
        # or ≤1 block/node, where reversal is invisible.) Reversing node-order while
        # preserving intra-node order also keeps a non-aligned split's shared boundary
        # block CONSECUTIVE, so the dedup below collapses it (fixes the ref-leak too).
        chains: list[list[KVBlock]] = []
        node: RadixNode | None = self
        while node is not None:
            chains.append(node.blocks)
            node = node.parent
        chains.reverse()  # root → self
        result = []
        for blocks in chains:
            result.extend(blocks)  # each node's blocks stay in their original order
        # A non-aligned split SHARES the boundary block between a parent's
        # LAST block and its child's FIRST block (for eviction safety), so it appears
        # TWICE consecutively here. That double-reads its KV during reconstruction AND
        # leaks ref_count (touch is per-occurrence → +2, but free dedups by block_id →
...
class RadixTree:
    ...
    Operates over Yunshu's UMA-resident block pool.
    ...
    def __init__(self, eviction_strategy: str = "lru", block_size: int = 64) -> None:
        ...
        # Match statistics for Prometheus monitoring
    ...
    def match(self, token_ids: list[int]) -> tuple[RadixNode, list[int]]:
        ...
        with self._lock:
            ...
            # Track match statistics for Prometheus export.
    ...
    def _split_node_unlocked(
        ...
        if not is_aligned and new_node.blocks:
            ...
            # Increment the block's ref_count to reflect the shared reference.
            # Without this, freeing either node's blocks would prematurely
            # release the block while the other node still depends on it.
            boundary_block.ref_count += 1
            if new_node.block_hashes:
                child.block_hashes.insert(0, new_node.block_hashes[-1])
            # If new_node has no block_hashes, the boundary block has no
            # hash to carry — this is valid for nodes created before hash
            # computation.  Skip inserting a boundary hash.
        ...
        if existing is not None:
            ...
            if match_len == 0:
                ...
                if len(token_ids) == len(existing.token_ids):
                    ...
                    remaining_new = token_ids[match_len:]
                    # When match_len is NOT block-aligned, the block covering
                    # the boundary straddles matched + remaining tokens. Using
                    # `len(existing.blocks)` (== floor) would hand that boundary block to
                    # the child, but the child's token_ids start mid-block while its
                    # blocks[0] starts at the block's first position → the child's KV
                    # would be MISALIGNED (block holds tokens N..N+bs but the node claims
                    # tokens N+k..). The tree has no per-block offset, so EXCLUDE the
    ...
    def evict(self, n_nodes: int) -> list[KVBlock]:
        ...
        with self._lock:
            ...
            while heap and evicted < n_nodes:
                ...
                # Boundary block safety: a non-aligned split (see
                # _split_node_unlocked) shares the boundary block between this
                # leaf and its parent and bumps the block's ref_count +1. If we
                # surrender that block here, the manager's free() drops it to
                # ref_count 0 + cache_only and re-queues it for allocation while
                # the PARENT still holds it in its prefix and is still matchable —
                # a subsequent allocate() would hand the block out and corrupt the
                # parent's cached KV. Exclude blocks the surviving parent still
                # holds, and undo the split's increment (mirrors the dedup in
                ...
                for _b in leaf.blocks:
                    if _b.block_id in _parent_block_ids:
                        _b.ref_count = max(0, _b.ref_count - 1)
                    else:
                        freed_blocks.append(_b)
    ...
    def _try_merge_unlocked(self, node: RadixNode) -> None:
        ...
        # A boundary block legitimately appears in both parent and child
        # after a split (ref_count was incremented during split).  When
        # merging, the dedup removes the child's copy — decrement ref_count
        # to avoid a permanent block leak.
        _seen: dict[int, tuple[Any, int]] = {}
        _deduped_blocks = []
        _deduped_hashes = []
        for b, h in zip(node.blocks, node.block_hashes, strict=False):
            if b.block_id in _seen:
                # Duplicate removed — undo the ref_count increment from split
                b.ref_count = max(0, b.ref_count - 1)
            else:
                _seen[b.block_id] = (b, h)
