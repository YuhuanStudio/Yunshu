...
def _mx_to_numpy(arr) -> np.ndarray:
    ...
    if isinstance(arr, np.ndarray):
        return arr
    # numpy has no bfloat16 dtype — np.array() on an MLX bf16 array raises
    # (item-size mismatch). Up-cast to float32 first (lossless: every bf16 value
    # is exactly representable in float32). The original dtype is recorded as
...
def _arr_dtype_str(orig, np_arr: np.ndarray) -> str:
    """Dtype string to persist for ``orig``. For MLX bfloat16 (which numpy can't
    hold) we store float32 bytes but record 'bfloat16' so the round-trip restores
    ...
    try:
        ...
        pass
    return _dtype_to_str(np_arr.dtype)


def _numpy_to_mx(np_arr: np.ndarray, dtype_str: str | None = None):
    """Convert numpy array back to MLX array (or keep as numpy in pure-test).

    When ``dtype_str == 'bfloat16'`` the buffer was stored as float32 (numpy has
...
def _str_to_dtype(s: str) -> np.dtype:
    ...
    if s == "bfloat16":
        # Stored as float32 bytes (numpy has no bf16); read as float32 then
        # _numpy_to_mx casts back to bfloat16.
...
class KVCacheSerializer:
    ...
    def serialize_block(
        ...
        key_dtype_str = _arr_dtype_str(key_data, key_np)
        val_dtype_str = _arr_dtype_str(value_data, val_np)
    ...
    def deserialize_block(self, data: bytes) -> tuple[KVBlock, object, object]:
        ...
        # Convert to MLX if available (restore bf16 if it was stored as float32)
        key_out = _numpy_to_mx(key_np, key_dtype_str)
        val_out = _numpy_to_mx(val_np, val_dtype_str)
    ...
    def serialize_table(
        ...
        key_dtype_str = _arr_dtype_str(key_cache, key_np)
        val_dtype_str = _arr_dtype_str(value_cache, val_np)
    ...
    def deserialize_table(self, data: bytes) -> tuple[BlockTable, object, object]:
        ...
        # Convert to MLX if available (restore bf16 if stored as float32)
        key_out = _numpy_to_mx(out_key, key_dtype_str)
        val_out = _numpy_to_mx(out_val, val_dtype_str)
