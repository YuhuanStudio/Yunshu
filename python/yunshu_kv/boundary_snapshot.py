    ...
    def _serialize(self, cache_state: dict) -> bytes:
        ...
        for key, value in cache_state.items():
            if key == "cache_type":
                ...
                # previously an unhandled value type (e.g. the `arrays`
                # LIST that ARRAYS_CACHE/CACHE_LIST states carry, or a None) was
                # SILENTLY dropped — num_entries stayed consistent so deserialize
                # wouldn't error, the key was just gone → silent KV state loss on
                # reload. Fail loudly instead so a future wiring of this store
                # (currently only .start()ed, never .save()d) can't corrupt
                # recurrent/hybrid snapshots undetected. The live hybrid path uses
                # HybridSnapshotStore (mx.save), not this serializer.
                raise TypeError(
                    f"BoundarySnapshot._serialize: unsupported value type for key "
                    f"{key!r}: {type(value).__name__}. This serializer handles only "
                    f"scalars/strings/arrays; list/None (ArraysCache/CacheList "
                    f"recurrent state) is not supported — use HybridSnapshotStore."
                )
    ...
    def _flush_pending_unlocked(self) -> None:
        ...
        for filepath_str, data in items:
            try:
                ...
                Path(tmp_path).write_bytes(data)
                os.replace(tmp_path, filepath_str)
                flushed_keys.append(Path(filepath_str).stem)
            except Exception as e:
                logger.warning(f"Boundary snapshot flush failed: {e}")
        # Mirror _writer_loop: drop persisted entries so the synchronous
        # backpressure flush doesn't leak _pending_writes (which would defeat
        # the _MAX_PENDING_WRITES memory bound). Lock order _write_lock →
        # _pending_lock matches; no opposite nesting exists.
        if flushed_keys:
            with self._pending_lock:
                for k in flushed_keys:
                    self._pending_writes.pop(k, None)
