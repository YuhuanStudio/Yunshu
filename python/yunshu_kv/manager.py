...
class KVCacheManager:
    ...
    - RadixTree-based prefix sharing (C8)
    ...
    def hit_rate(self) -> float:
        ...
        Snapshot both counters under `_lock` so the ratio
        is consistent. Previously read without lock → on rare interleave
        a metrics scrape could see hits > lookups (transient > 1.0).
        """
        with self._lock:
            _lookups = self._total_lookups
            _hits = self._total_hits
        if _lookups == 0:
            return 0.0
        return _hits / _lookups
    ...
    def _allocate_for_prefill_unlocked(
        ...
        if self.config.enable_caching and len(token_ids) >= self.config.block_size:
            ...
            # Validate matched blocks under block_pool._lock to prevent TOCTOU:
            # between checking ref_count/cache_only and calling touch(), a
            # concurrent eviction thread could free the block.  block_pool._lock
            # is an RLock, so re-entrant calls to touch() and allocate() inside
        ...
        for h in block_hashes:
            ...
            if cached is not None:
                ...
                self._total_hits += 1
            elif self._warm_tier is not None:
                # Warm tier: attempt promote directly (avoids TOCTOU between
                # contains() and promote() — promote() returns None if evicted).
                try:
                ...
                if promoted is not None:
                    ...
                    try:
                        ...
                        # may be reallocated to a different request (dangling
                        # reference → data corruption).
                        self.block_pool.evict_and_free(new_block)
                        # evict_and_free leaves a ref_count==1 block un-freed
                        # (it expects an owning request to free it later). This
                        # error path aborts before the block joins any
                        # BlockTable, so drop the ref here or it leaks.
    ...
    def cache_to_radix_tree(
        ...
        """Insert completed blocks into the RadixTree (C8).
    ...
    def _evict_for_memory_unlocked(self, needed_blocks: int) -> bool:
        ...
        # evict() returns blocks from evicted tree nodes. Some were already
        # freed by evict_and_free() above — skip those to avoid double-free.
        # Any remaining blocks with ref_count > 0 are leaked (tree node gone
    ...
    def get_tier_stats(self) -> dict:
        ...
        """
        # snapshot the three counters together under the lock so
        # total_lookups / total_hits / hit_rate are mutually consistent (was a
        # cosmetic torn read — reading hits/lookups unlocked then re-locking in
        # the hit_rate property could report disagreeing values).
        with self._lock:
            _lookups = self._total_lookups
            _hits = self._total_hits
        _hit_rate = (_hits / _lookups) if _lookups else 0.0
        hot_stats = {
            "usage_pct": round(self.usage * 100, 1),
            "free_blocks": self.num_free_blocks,
            "block_size": self.block_size,
            "total_lookups": _lookups,
            "total_hits": _hits,
    ...
    def save_prefix(self, prefix_hash: int, path: str) -> None:
        ...
        if self._key_cache is None or self._value_cache is None:
            raise RuntimeError(
                "KV tensors not bound — call set_kv_tensors() before save_prefix()"
            )
    ...
    def save_all_cached(self, path: str) -> None:
        ...
        if cached_blocks and (self._key_cache is None or self._value_cache is None):
            raise RuntimeError(
                "KV tensors not bound — call set_kv_tensors() before save_all_cached()"
            )
