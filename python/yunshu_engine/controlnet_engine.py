...
class ControlNetBlock:
    ...
    def inject_condition(
        ...
        # Default: additive-bias conditioning (no ControlNet weights loaded)
        ...
        # CRITICAL: normalize the conditioning to the noise-latent scale before
        # adding it. The raw VAE-encoded condition latents are off-distribution
        # relative to the unit-normal flow-matching latents the transformer
        # expects; adding them directly pushes the transformer input far
        # off-manifold and collapses the decoded image to black (verified:
        ...
        # bias into the late refinement steps only pushes the output darker and
        # darker (verified: full-trajectory bias at strength=1.0 collapsed to
        # mean≈3/255). Apply it strongly early and linearly fade to zero by 60%
        # of the schedule so later steps refine on-distribution.
        fade = max(0.0, 1.0 - step_frac / 0.6)
        scaled_condition = condition_norm * control_strength * 0.05 * fade
