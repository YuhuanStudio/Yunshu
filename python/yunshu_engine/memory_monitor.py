...
def get_max_working_set_bytes() -> int:
    ...
    # Fallback: Metal max_recommended_working_set_size is typically ~66-70%
    # of system RAM (GPU shares unified memory with CPU).  Using 75% would
    # overshoot and risk eviction stalls.  0.65 is conservative enough to
    # avoid Metal working set pressure on all shipping Apple Silicon configs.
    return int(get_system_memory() * 0.65)
