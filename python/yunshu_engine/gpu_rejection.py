...
"""GPU-accelerated rejection sampling for speculative decoding.
...
class BatchRejectionResult:
    ...
    latency_us: float
    resampled_token_id: int | None = None
...
class GPURejectionSampler:
    ...
    def verify_greedy(
        ...
        rejection_position = accepted_count if accepted_count < K else None
    ...
    def verify_stochastic(
        ...
        # Step 6: Resample on rejection.
        # verify_stochastic only receives TOKEN-LEVEL draft logprobs —
        # draft_lps is [K], the draft prob of the CHOSEN token at each position, NOT
        # the full draft distribution over the vocab. The true speculative-sampling
        # residual max(0, p_target(x) - p_draft(x)) needs p_draft for EVERY x, which
        # we don't have here. The previous code broadcast-subtracted the single scalar
        # p_draft(rejected) from every vocab entry — a mathematically meaningless
        # distribution that did NOT preserve the target. The documented-correct
        # fallback when only token-level draft probs are available (see
        # spec_draft_verifier._sample_correction) is to sample directly from the
        # target distribution, which preserves target fidelity. categorical() takes
        # logits, and a logprobs row is a valid (additively-shifted) logit row.
        if rejection_position is not None:
            i = rejection_position
            resampled_token_id = int(
                mx.random.categorical(target_logprobs_full[i].reshape(1, -1)).item()
            )
