...
class RequestLifecycleOrchestrator:
    ...
    def on_prefill_start(self, request_id: str) -> bool:
        ...
        if state.transition(RequestPhase.PREFILLING):
            ...
            # the request has LEFT the QUEUED state → drop it from _pending_queue.
            # The old code only removed ids on the failed/aborted/timeout paths, so every
            # request that queued-while-over-limit and then SUCCEEDED leaked its id forever.
            # Unbounded growth + once len>=_max_pending (256) every later over-limit add hit
            # the REJECTED branch → _total_rejected and the pending/SLO stats poisoned
            # permanently.
    ...
    def on_request_finished(
        ...
        # defensive — a request that finished WITHOUT going through
        # on_prefill_start (e.g. a fast completion) must not leave a stale id in
        # _pending_queue (sibling of the success-path leak fixed in on_prefill_start).
        with contextlib.suppress(ValueError):
            self._pending_queue.remove(request_id)
