...
def _stable_repr(obj: Any) -> str:
    ...
    Note: bool is a subtype of int in Python, but ``repr(True) == 'True'`` and
    ``repr(1) == '1'`` are distinct, so True/1 and False/0 hash to different
    keys here.  This is intentional — callers that want bool/int conflation
    must normalise before calling.
    """
    if obj is None:
        return "None"
    # Check bool explicitly first (bool is a subclass of int).  This is purely
    # for clarity: repr() already differentiates them, but explicit ordering
    # documents the intent and survives future refactors.
...
class ModelPreprocessorRegistry:
    ...
    def get_preprocessor(self, model_id: str, modality: str) -> ProcessorFn | None:
