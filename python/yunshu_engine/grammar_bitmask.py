...
"""Grammar bitmask engine for structured output — token masking.
...
class BitmaskApplicator:
    ...
    def apply(self, logits: Any, bitmask: Any) -> Any:
        ...
        # Align bitmask vocab size with logits vocab size. Tokenizer vocab
        # (e.g. 151643 for Qwen2.5) is often smaller than the logits' final
        # axis (151936 with padding/added tokens). Pad with
        # True (BLOCKED), not False. The grammar's allowed set is always within the
        # tokenizer vocab [0, mask_vocab); the extra logits positions are padding/added/
        # special tokens the grammar never authorized. Padding them False (allowed) let
        # a high-logit added token (Qwen carries real logits on ~293 such positions)
        # outscore the grammar-allowed tokens and break the JSON/regex structure. The
        # all-blocked safety net below still guarantees ≥1 finite logit.
        logits_vocab = logits.shape[-1]
        mask_vocab = blocked.shape[-1]
        if mask_vocab != logits_vocab:
            if mask_vocab < logits_vocab:
                pad_len = logits_vocab - mask_vocab
                padding = mx.ones((pad_len,), dtype=blocked.dtype)
                blocked = mx.concatenate([blocked, padding], axis=-1)
            else:
                blocked = blocked[..., :logits_vocab]
        neg_inf = mx.array(float("-inf"), dtype=logits.dtype)
    ...
    def apply_allowlist(self, logits: Any, allowed_ids: list[int]) -> Any:
        ...
        if allowed_ids:
            allowed_ids = [tid for tid in allowed_ids if 0 <= tid < self._vocab_size]
            if allowed_ids:
...
class GrammarBitmaskEngine:
    ...
    def compute_bitmask(self, tokenizer: Any) -> Any:
        ...
        # Bounds-check: vocab size may differ between models
        allowed_ids = [tid for tid in allowed_ids if 0 <= tid < vocab_size]
        if not allowed_ids:
            return mx.zeros((vocab_size,), dtype=mx.bool_)
    ...
    def get_stats(self) -> dict[str, Any]:
        stats: dict[str, Any] = getattr(self._constraint, "get_stats", lambda: {})()
