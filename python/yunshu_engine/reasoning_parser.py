...
class ReasoningParser(ABC):
    ...
    def parse(self, text: str) -> ReasoningOutput:
        ...
        # CLOSING-ONLY case: Qwen3/Qwen3.5/DeepSeek-R1 chat templates inject the
        # opening <think> themselves, so the MODEL output frequently contains only
        # the closing </think> (no opening tag). Treat everything up to that first
        # close as reasoning (the open was templated). Without this the whole CoT +
        # the literal </think> leaked into content and reasoning_tokens was 0 — the
        ...
        if reasoning_parts:
            ...
            # Strip any residual think tags from content. In the closing-only
            # case (templated <think>), a SECOND unmatched </think> emitted inside the
            # answer (e.g. "r1</think>mid</think>tail") was dumped to content verbatim —
