...
class MCPServerConnection:
    ...
    def __init__(self, config: MCPServerConfig) -> None:
        ...
        # WRONG result returned to the wrong caller). Serialize each request/response round
        # trip; combined with id-matching this makes the transport concurrency-safe.
    ...
    async def _initialize(self) -> None:
        """MCP spec: a client MUST send ``initialize`` (then the ``notifications/initialized``
        notification) before any other request. Previously we jumped straight to ``tools/list``,
        so a spec-conformant server rejected/ignored it → ``self.tools`` stayed empty → the tool
    ...
    async def _send_stdio(self, method: str, params: dict) -> Any:
        ...
        # Serialize the whole write+read round trip on the shared pipe.
        async with self._stdio_lock:
            self._process.stdin.write(msg.encode())
            await self._process.stdin.drain()

            if self._process.stdout:
                # Read lines until we get the response whose id MATCHES our request
                # (skip notifications, and skip a stray response for a different id).
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.debug("MCP stdio: skipping non-JSON line")
                        continue
