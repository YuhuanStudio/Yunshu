...
class TeaCacheHook:
    ...
    def forward(self, transformer, x, timestep, sigmas, cap_feats):
        ...
        if not should_run and self.state.previous_residual is not None:
            ...
            self._cache_hits += 1
            # Increment cnt on BOTH paths so it is a true step index (incremented on
            # every forward). Returning early here would skip the cnt += 1 below, so cnt
            # would count only MISSES. Harmless today (cnt is only consulted via the
            # cnt==0 first-step special case, and step 0 always takes the slow path), but a
            # latent foot-gun for any future cnt-based decision (last-step / cnt % k).
            self.state.cnt += 1
