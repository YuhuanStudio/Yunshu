...
class RequestPriorityQueue[T]:
    ...
    def pop_lowest_priority(self) -> T | None:
        """Remove and return the LOWEST-priority (or newest) item.

        Opposite of pop(): in FCFS mode returns the most recently inserted item;
        in PRIORITY mode returns the lowest-priority item (ties broken by newest).
        Used by the memory guard to reject the least important waiting request
        when the scheduler is stuck in a deferral spin-loop.
        ...
        with self._lock:
            if not self._heap:
                return None
            # heappop gives us the min (highest priority). We want the max
            # (lowest priority).  The max is at the end of a sorted list,
            # but heapq doesn't maintain the full sort.  Scan for it.
            # For small heaps this is O(n) which is fine for the rejection path.
            max_idx = 0
            for i in range(1, len(self._heap)):
                if self._heap[i] > self._heap[max_idx]:
                    max_idx = i
            entry = self._heap[max_idx]
            # Swap with last element and pop (O(1) removal from end)
            self._heap[max_idx] = self._heap[-1]
            self._heap.pop()
            if self._heap and max_idx < len(self._heap):
                heapq._siftdown(self._heap, 0, max_idx)
                heapq._siftup(self._heap, max_idx)
            return self._extract_item(entry)
