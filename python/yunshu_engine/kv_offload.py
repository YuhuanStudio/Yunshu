...
class KVOffloadManager:
    ...
    def __init__(
        ...
        if self._bytes_per_block == 0 and kv_manager is not None:
            block_size = getattr(kv_manager, 'block_size', config.block_size)
    ...
    async def promote_block(
        ...
        # Miss
        self._stats.total_promotion_misses += 1
        return None
    ...
    def _execute_offload(self, request: OffloadRequest) -> OffloadResult:
        ...
        self._stats.total_blocks_offloaded += result.blocks_offloaded
        self._stats.total_blocks_skipped += result.blocks_skipped
        self._stats.total_blocks_failed += result.blocks_failed
        self._stats.total_bytes_offloaded += result.bytes_offloaded
        self._stats.offload_latencies.append(result.latency_seconds)
        if len(self._stats.offload_latencies) > 1000:
            self._stats.offload_latencies = self._stats.offload_latencies[-500:]
        self._stats.last_offload_step = self._step_counter
        self._stats.last_offload_time = time.monotonic()
    ...
    async def _wait_for_completion(self, request_id: str) -> OffloadResult:
        ...
        raise asyncio.TimeoutError(f"Offload {request_id} did not complete in {max_wait}s")
    ...
    async def _async_worker(self) -> None:
        ...
        try:
            while self._running:
                ...
                result = self._execute_offload(request)
