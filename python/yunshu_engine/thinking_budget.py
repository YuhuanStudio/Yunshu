...
Thinking budget is enforced by injecting a stop sequence
...
# The real Qwen3/Qwen3.5/DeepSeek-R1/GLM reasoning markers are <think> and
# </think> (NO slash). The slash form "<think/>" is a self-closing XML tag no model emits,
# so convert_tokens_to_ids("<think/>")→None / encode("</think/>")→multi-token garbage →
# detect_needs_think_prefix never pre-seeded _in_thinking on the streaming path and the WHOLE
# chain-of-thought leaked into delta.content.
THINK_START = "<think>"
THINK_END = "</think>"
...
def _resolve_think_end_token_ids(tokenizer) -> list[int] | None:
    ...
    try:
        return tokenizer.encode("</think>", add_special_tokens=False)  # no slash
    except Exception:
