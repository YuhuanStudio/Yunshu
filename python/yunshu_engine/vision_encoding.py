...
Mirrors oMLX's 3 vision encoding strategies (encode_image, qwen, llava).
...
class VisionEncodingStrategy(StrEnum):
...
class LLaVAEncoder(VisionEncoder):
    ...
    def _anyres_preprocess(self, image: Any, processor: Any) -> mx.array | None:
    ...
    def supports_model(self, model: Any) -> bool:
        ...
        module = type(model).__module__
        return bool(isinstance(module, str) and "llava" in module)
...
class VisionEncoderFactory:
    ...
    @classmethod
    def _auto_detect(cls, model: Any, **kwargs) -> VisionEncoder | None:
