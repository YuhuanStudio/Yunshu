...
class TokenLevelScheduler:
    ...
    def compute_token_budget(
        ...
        if n_prefill == 0:
            ...
            # 60/40 split favoring PREFILL (int(total*0.6) → prefill).
            # The old comment said "favoring decode", the opposite of the code; decode
            # still gets >=40% here, satisfying the docstring's "at least 20% to
            # decode". (This split is stats-only — the return is discarded by the
            # decode path — so only the comment was wrong, not an allocation.)
            prefill_budget = max(
