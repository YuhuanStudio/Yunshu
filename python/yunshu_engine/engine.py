...
from .types import (  # noqa: F401 — re-export for backward compat
    EngineConfig,
    RequestPhase,
)
...
class RequestState:
    ...
    uid: int | None = None  # BatchGenerator-assigned UID
    prompt_token_count: int = 0
    completion_token_count: int = 0
    generated_text: str = ""
    finish_reason: str | None = None

    # Output token IDs for cumulative decode
...
# Module-level deferred clear state.
...
class Engine:
    ...
    async def stop(self) -> None:
        ...
        # Release model and tokenizer references
        self._model = None
        self._tokenizer = None
    ...
    def _create_detokenizer(self):
        ...
        Do NOT pool detokenizer instances.
    ...
    async def _step_loop(self) -> None:
        ...
        while self._running:
            ...
            # 5. Deferred Metal cache cleanup
    ...
    def _distribute_responses(self, responses: list) -> None:
        ...
        for resp in responses:
            ...
            # Decode token via per-request detokenizer
            token_text = ""

            if not is_stop:
                # Track output token ID
            ...
            if finish_reason:
                # Finalize detokenizer to flush remaining bytes
                if state.detokenizer is not None:
                    try:
    ...
    def _messages_to_text(
        ...
        Uses the tokenizer's apply_chat_template:
    ...
    def _init_memory_monitor(self) -> None:
        """Initialize memory monitor with model architecture info.
