...
class BatchSampler:
    ...
    def sample_batch(
        ...
        # Filter on the UN-tempered logits and
        # apply temperature LAST. top_p/min_p compute softmax/log thresholds
        # internally, so dividing by temperature FIRST shifts the nucleus / min_p
        # threshold to a different surviving token set than mlx-lm make_sampler
        # (which applies temp last) and the non-streaming fast path — the
        # engine-loop batch path would diverge from both for temp!=1 + a filter.
        # (top_k is scale-invariant, so its position doesn't matter.)
        # Step 1: Top-k filtering — per-request masking
        t_topk = time.perf_counter()
        filtered = self._apply_batch_top_k(logits_batch, plan.top_k_values)
        self._record_op("top_k_filter", t_topk)

        # Step 2: Top-p filtering — per-request nucleus sampling (un-tempered)
        t_topp = time.perf_counter()
        filtered = self._apply_batch_top_p(filtered, plan.top_p_values)
        self._record_op("top_p_filter", t_topp)

        # Step 3: Min-p filtering — per-request dynamic threshold (un-tempered)
        t_minp = time.perf_counter()
        filtered = self._apply_batch_min_p(filtered, plan.min_p_values)
        self._record_op("min_p_filter", t_minp)

        # Step 4: Temperature scaling LAST — masked (-inf) positions stay masked.
        t_temp = time.perf_counter()
        scaled = filtered / plan.temperatures
...
class LogitsProcessorBatch:
    ...
    def _apply_repetition_penalty(
        ...
        logits >= 0 are divided by penalty.
        """
        penalty = config.repetition_penalty
        # penalty <= 0 makes the `logits / penalty` branch divide by zero
        # (→ ±inf, then NaN through softmax → the sampler picks garbage / crashes).
        # The gateway schemas allow ge=0.0, so a caller can pass exactly 0.0; treat any
        # non-positive penalty as a no-op (the only meaningful values are penalty > 0,
        # with >1 penalizing repeats and <1 encouraging them).
    ...
    def _apply_grammar_bitmask(
        ...
        # bitmask is True where tokens are allowed. Align its vocab to the
        # logits vocab first — tokenizer vocab (e.g. 151643) is often smaller
        # than the padded logits final axis (e.g. 151936); without this the
        # mx.where below raises a broadcast error and crashes every
        # grammar-constrained batched request. Pad with True (allowed) so the
        # extra padding tokens aren't masked. Mirrors BitmaskApplicator.apply.
        bm = bitmask.reshape(1, -1)
        logits_vocab = logits_row.shape[-1]
        mask_vocab = bm.shape[-1]
        if mask_vocab != logits_vocab:
            if mask_vocab < logits_vocab:
                pad = mx.ones((1, logits_vocab - mask_vocab), dtype=bm.dtype)
                bm = mx.concatenate([bm, pad], axis=-1)
            else:
                bm = bm[..., :logits_vocab]
        result = mx.where(
            bm,
...
class _AhoCorasickTrie:
    ...
    def get_pattern(self, index: int) -> tuple[tuple[int, ...], int, str]:
        ...
        The typed `_patterns` signature allows 2-tuples (no
        original_string); pad to 3-tuple for callers.
        """
    ...
    def get_pattern_token_count(self, index: int) -> int:
        ...
        # Handle 2-tuples so the unpack here doesn't
        # ValueError if a future caller constructs _patterns with 2-tuples.
