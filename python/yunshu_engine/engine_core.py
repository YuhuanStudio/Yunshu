...
"""Yunshu EngineCore — continuous batching orchestrator.
...
- stream_outputs() uses the get_nowait() or await get() pattern
...
class EngineCoreConfig:
    ...
    # Batch-accumulation window (ms): on the idle→active transition, wait up to
    # this long for a BURST of concurrent requests to all queue before the first
    # scheduler step, so they batch together. Without it, HTTP concurrency trickles
    # requests in (each awaits add_request on the single executor, racing the
    # loop's step) → the first request decodes ALONE and the rest land in later
    # under-filled steps → non-deterministic throughput collapse under load
    # (in-process insert() adds all N at once and never sees this). Polls early-exit
    # once prefill_batch_size are queued, so single-request TTFT is barely affected.
    batch_wait_ms: float = 15.0
    ...
    # scheduling policy. "fcfs" (default), "priority" (higher
    # sampling_params.priority scheduled first, preempts lower-priority running
...
class EngineCore:
    """MLX-native continuous batching engine core.
    ...
    def __init__(
        ...
        scheduler_config = SchedulerConfig(
            policy=_policy,
            aging_weight=getattr(self.config, "aging_weight", 0.1),
            ...
            if not has_arch:
                ...
                try:
                    ...
                    if ssd_dir:
                        ...
                        # namespace the SSD dir per model. Cross-model disk-KV pollution was
                        # fixed for the DEFAULT path (SSDKVCache) but never propagated to this
                        # tiered/paged SSDCacheStore — it keyed blocks by content hash alone
                        # with model_hash always 0, so two models sharing one
                        # YUNSHU_SSD_CACHE_DIR + a common token prefix would serve each other's
                        # KV (garbage / shape-mismatch crash). Reuse the per-model scoping.
                        # The model name isn't plumbed into EngineCoreConfig, so
        ...
        # completion_batch_size from 32 to ~1 and never recovered — silently neutralizing
        ...
        self._slo_monitor.set_auto_tuning_callback(
            lambda violations: self._on_slo_violation(violations)
        )
    ...
    def _set_completion_batch_size(self, n: int) -> None:
        """Write the tuned completion_batch_size to BOTH the EngineCoreConfig
        (read by the token-scheduler + spec budgets) AND the scheduler's OWN
        SchedulerConfig copy (read by _effective_max_seqs admission + the spec slot cap).

        At init the value is copied BY VALUE into a fresh SchedulerConfig, so mutating
        only self.config left the scheduler's admission cap frozen at the original —
        adaptive batch sizing / AutoTuner (incl. the ratchet) were no-ops for admission,
        and the two budgets silently drifted apart (token/spec budgets followed the live
        value; admission did not). Sync both so a tuned value actually throttles
        admission. (The mlx-lm BatchGenerator bakes the decode width at construction and
        isn't rebuilt, so the per-step DECODE width still can't retune live — a deeper
        limitation; admission + budgets now stay consistent.)"""
        self.config.completion_batch_size = n
        _sched = getattr(self, "scheduler", None)
        _scfg = getattr(_sched, "config", None) if _sched is not None else None
        if _scfg is not None and hasattr(_scfg, "completion_batch_size"):
            _scfg.completion_batch_size = n
        # the spec-aware scheduler snapshots its max_num_seqs at init as the EFFECTIVE
        # cap min(max_num_seqs, completion_batch_size). Syncing the admission cap alone
    ...
    def _apply_tuning_to_config(self, decisions: list) -> None:
        ...
        try:
            ...
            if params.batch_size != self.config.completion_batch_size:
                ...
                self._set_completion_batch_size(max(1, min(params.batch_size, 64)))
            ...
            if spec_dec is not None and hasattr(spec_dec, "config"):
                ...
                if new_dl != spec_dec.config.draft_length:
                    ...
                    spec_dec.config.draft_length = new_dl
                    logger.info("AutoTuner: spec_draft_length %d -> %d", old, new_dl)
        except Exception:
            logger.debug("auto-tuner config application failed", exc_info=True)

    def _on_slo_violation(self, violations: list) -> None:
        """Handle persistent SLO violations by triggering auto-tuning.

        Called from SLOMonitor.check_slo() when violations exceed threshold.
        Runs a single auto-tune cycle and applies any decisions.
        """
        try:
            logger.info(
                "SLO violation callback: %d recent violations, triggering auto-tune",
                len(violations),
            )
            decisions = self._auto_tuner.auto_tune()
            if decisions:
                self._apply_tuning_to_config(decisions)
        except Exception:
            logger.debug("SLO violation auto-tune failed", exc_info=True)
    ...
    async def add_request(
        ...
        **kwargs,
        ...
        # Reject new requests during graceful shutdown (REQUESTED state)
        ...
        # do NOT acquire when the engine-loop (batched) mode is running. Per-request
        # LoRA is unsupported there (the model is shared across the batch, so in-place LoRA
        # would corrupt concurrent requests — the request is rejected below). The old code
        # acquired the adapter ANYWAY and then immediately released it, and acquire_adapter
        # mutates the shared _base_model in-place ON THE EVENT-LOOP THREAD — racing the
        ...
        if lora_adapter and not loaded_lora and getattr(self, "_running", False):
            ...
            self._output_collectors[req_id].put(None)
            self._finished_events[req_id].set()
            self._request_lora_adapters.pop(req_id, None)
            return req_id
        ...
        # Inflight prefix sharing: register for concurrent KV prefix reuse.
        ...
        # ── KV lifecycle admission ──
        _block_id = next(self._block_id_counter)
        self._request_block_ids[req_id] = _block_id
        try:
    ...
    async def stream_outputs(
        ...
        whose clients have already disconnected.
        """
        collector = self._output_collectors.get(request_id)
        ...
        try:
            ...
            if _cancelled:
                # Propagate cancellation to scheduler.
    ...
    async def generate(
        ...
        **kwargs,
        ...
        try:
                    ...
                    if _cancel_event is not None:
                        ...
                        try:
                            while True:
                                ...
                                # only cancel pending tasks we are
                                # about to replace (the _cancel_waiter when
                                # the cancel event hasn't fired).  Previously
                                # this loop cancelled _wait_task too, which
                                # would silently abort an in-flight wait on
                                # the completion event.  We now leave
                                # _wait_task alive across iterations and only
                                # cancel the prior _cancel_waiter explicitly
                                # before reassignment below.
                                if _cancel_waiter in done:
                                    if isinstance(_cancel_event, asyncio.Event):
                                    ...
                                    # Cancel the prior cancel_waiter before
                                    # reassigning — otherwise repeated loop
                                    # iterations on non-asyncio.Event cancel
                                    # signals could pile up tasks.  The
                                    # already-done case is harmless (cancel()
                                ...
                                # returned via timeout — raise TimeoutError
                                # to match the asyncio.wait_for branch.
                                if not done:
                                    if not _wait_task.done():
                                        _wait_task.cancel()
                                    if not _cancel_waiter.done():
            ...
            # Drain collector — always merge so we accumulate ALL tokens.
            result = None
            if collector:
                while True:
                    output = collector.get_nowait()
            ...
            if result is not None:
                ...
                result = RequestOutput(
                    ...
                    finished=True,
                    finish_reason="error",
                    error="No output received (engine may have shut down)",
                    prompt_tokens=0,
                    completion_tokens=0,
                )
    ...
    async def _engine_loop(self) -> None:
        """Main engine loop — drives continuous batching.
        ...
        while self._running:
            ...
            if not self.scheduler.has_requests():
                ...
                if self._wake_event is not None:
                    ...
                    # asyncio.Event.wait() does NOT auto-clear — without this
                    # clear() the event stays set
                    # after the first wake and the loop busy-spins at 100% CPU. A
                    # lost wake is harmless: the next iteration re-checks
                    # has_requests() and the 100ms fallback re-polls regardless.
                    self._wake_event.clear()
                else:
                    await asyncio.sleep(self.config.step_interval)
                self._total_idle_time_ms += (time.monotonic() - _idle_start) * 1000
                # Batch-accumulation window (idle→active only): a burst of concurrent
                # requests each reach scheduler.add_request at slightly different times;
                # stepping immediately on the FIRST wake decodes it alone while the rest
                ...
                if _bw > 0 and self.scheduler.has_requests():
                    ...
                    # break early ALSO when the queue STOPS GROWING — the old code
                    # only early-exited at prefill_batch_size, so a genuinely-idle SINGLE
                    # request (1 < batch_size) waited the FULL batch_wait_ms (the "single-
                    # request TTFT barely affected" claim only held for the warm-loop case).
                    # A real burst keeps arriving (count climbs) so it still accumulates; an
                    # isolated request sees a stable count for ~2 cycles and proceeds → its
                    # window penalty drops from batch_wait_ms to ~4ms.
                    try:
                        _last_n = len(self.scheduler.waiting)
                    except Exception:
                        _last_n = 0
                    _stable = 0
                    while time.monotonic() < _accum_deadline:
                        try:
                            _n = len(self.scheduler.waiting)
                            if _n >= self.config.prefill_batch_size:
                                break
                            if _n > _last_n:
                                _stable = 0  # still arriving → keep accumulating
                                _last_n = _n
                            else:
                                _stable += 1
                                if _stable >= 2:  # queue stable ~2 cycles → burst done
                                    break
                        except Exception:
                            break
            ...
            try:
                ...
                # CompositionScheduler pre_step hooks (metrics, memory pressure)
                if self._composition_scheduler is not None:
                    try:
                        ...
                        for m in self._composition_scheduler._mixins:
                            if isinstance(m, MemoryPressureMixin):
                                ...
                                if 0 < rec_batch < self.config.completion_batch_size:
                                    ...
                                    # — a memory-pressure shrink that never reached
                                    # _effective_max_seqs let admission keep over-filling.
                                    self._set_completion_batch_size(max(4, rec_batch))
                                break
                    except Exception:
                        logger.debug("composition pre_step failed", exc_info=True)

                # a priority INVERSION is a high-priority WAITING
                # request blocked by a low-priority RUNNING one — it cannot exist
                # when nothing is waiting. The token-level scheduler likewise only
                # makes an admission/budget decision when there are requests to
                # admit. In steady-state batched decode (no waiting) both are pure
                # no-ops, so skip their O(running) SchedulableRequest rebuilds +
                # checks entirely — this is the dominant per-step orchestration
                # overhead vs a static batcher. Boost EXPIRY is unaffected: the
                # restoration loop below calls get_boost(), which pops expired
                # boosts. No behavior change, only skipped no-op work.
                _n_waiting = (
                ...
                # only meaningful when admitting/budgeting WAITING
                # requests; its output is not consumed by the decode path (stats
                ...
                try:
                    ...
                    # apply the suggestion in BOTH directions, clamped to
                ...
                if (
                    ...
                    try:
                        ...
                        logger.error("Failed to recreate executor", exc_info=True)
            ...
            try:
                ...
                # Materialize outputs into a list to prevent generator
                # exhaustion in the error handler's re-iteration below.
                if not isinstance(scheduler_output.outputs, list):
                    scheduler_output.outputs = list(scheduler_output.outputs)
                ...
                if scheduler_output.outputs:
                    # ── Lifecycle decode tracking for active requests ──
                    for req_output in scheduler_output.outputs:
                        rid = req_output.request_id
                            ...
                            if self._sliding_window_mgr is not None:
                                try:
                                    ...
                                    if evicted:
                                        ...
                                        # updates its own logical bookkeeping and trims
                                        # MLX KV arrays, but the physical KVBlock objects
                    ...
                    try:
                        ...
                        # Prune _ttft_done entries whose requests have left the
                        # scheduler — prevents unbounded growth across engine lifetime.
                        if len(self._ttft_done) > 2048:
                            _active_rids = (
                                set(self.scheduler.running)
                                | set(self.scheduler.waiting)
                                | set(self._output_collectors.keys())
                            )
                            self._ttft_done &= _active_rids
                        ...
                        if timeout_s > 0:
                            ...
                            for rid in list(self._request_timestamps.keys()):
                                ...
                                if (now - start) > timeout_s:
                                    ...
                                    # the collector and finished_event before the consumer
                    ...
                    try:
                        ...
                        # Decrement prefill_batch_size halving cooldown
                        if self._pbs_halve_cooldown > 0:
                            self._pbs_halve_cooldown -= 1
    ...
    def _fail_active_requests(self, error_msg: str) -> None:
        ...
        so entries don't leak if consumers have already detached.
        """
        from .request import RequestOutput
        ...
        # Without this snapshot, _finalize_request adds each req_id
        # to _finalized_ids inside the loop, so the post-loop check
        # finds ALL ids already finalized and reports 0 new failures.
        _pre_finalize = set(self._finalized_ids)
        ...
        for req_id in all_ids:
            ...
            # release consumer-side state (collectors, events, timestamps,
            # stream states, prefix hashes).  If consumers are still
            # running, they hold a local reference to the collector and
            # can drain it; _cleanup_request only removes the dict entry.
            # If consumers have already detached (timeout/cancel), this
            # prevents permanent memory leaks in per-request tracking dicts.
            self._cleanup_request(req_id)

        # Count failed requests so get_stats() and Prometheus report
        # accurate totals.  Deduplicate against pre-finalize snapshot
        # to avoid double-counting requests already finished in the
        # normal path before the exception triggered _fail_active_requests.
        new_fails = [rid for rid in all_ids if rid not in _pre_finalize]
        if new_fails:
    ...
    def _tbo_step(self) -> Any:
        """Run one scheduler step with Two-Batch Overlap.
...
class AsyncEngineCore:
    """Async context manager wrapper around EngineCore.
