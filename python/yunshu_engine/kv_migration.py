...
class KVMigrationManager:
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            ...
            # Be honest about what these counters mean.
            # When the engine registers blocks for temperature tracking it does
            # NOT pass the KV payload (register_block(..., data=None)), so a
            # "gpu_to_ssd" here is a LOGICAL tier/temperature transition used for
            # eviction policy — NOT a physical disk write (avg_migration_time_s
            # ~1e-5 confirms no real I/O). Actual KV persistence to disk is done
            # by the KVPrefixCache SSD tier (SSDKVCache), reported separately
            # under /monitoring/ssd-cache. Don't read total_bytes_transferred as
            # bytes written to SSD.
            stats["physical_io"] = any(
                getattr(t, "byte_size", 0) and getattr(t, "_payload", None) is not None
                for t in self._temperatures.values()
            )
            stats["note"] = (
                "tier_counts/*_count are LOGICAL temperature transitions for "
                "eviction policy, not physical I/O; real SSD persistence is the "
                "prefix-cache SSD tier (/monitoring/ssd-cache)."
            )
            # NOTE: Prometheus metrics are set by the gateway monitoring
            # endpoints (metrics.py /metrics and monitoring.py /prometheus)
            # with proper model_id labels.  Do NOT push from get_stats()
            # to avoid creating unlabeled series alongside the labeled ones.
            return stats
...
class MultiTierCacheCoordinator:
    ...
    def _evict_if_full(self, tier: KVTier) -> int:
        ...
        evicted = 0
        # `store and` guards capacity==0: otherwise `0 >= 0` stays true on an
        # empty store and popitem() raises KeyError.
        while store and len(store) >= capacity:
            # Evict oldest (LRU)
