...
Integration:
  ...
  - Distributed coordinator splits diffusion work across multiple nodes
...
class DiffusionLoRAOffloader:
    ...
    def load_for_step(self, step: int, lora_ids: list[str] | None = None) -> list[str]:
...
class DistributedDiffusionCoordinator:
    ...
    def get_last_checkpoint(self) -> SyncCheckpoint | None:
        """Get the most recent checkpoint for fault-tolerant restart."""
        if not self._checkpoints:
            return None
        return self._checkpoints[-1]

    def get_restart_assignment(self) -> NodeAssignment | None:
