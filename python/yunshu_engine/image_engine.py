...
Self-implemented Z-Image/Flux-family diffusion pipeline.
...
class TextEncoder(nn.Module):
    ...
    Returns second-to-last hidden state.
    """
...
# Model-agnostic diffusion feature machinery lives in the middle layer; this
# module keeps only the thin Z-Image adapter (module resolution, key remap,
# tokenize). See diffusion_features.py.
from .diffusion_features import (
    apply_prompt_weights as _apply_prompt_weights,
)
from .diffusion_features import (
    build_token_weights as _build_token_weights,
)
from .diffusion_features import (
    load_diffusion_lora as _df_load_diffusion_lora,
)
from .diffusion_features import (
    parse_lora_tags as _parse_lora_tags,
)
from .diffusion_features import (
    parse_prompt_attention as _parse_prompt_attention,
)
from .diffusion_features import (
    resolve_lora_file as _df_resolve_lora_file,
)
...
class _RopeEmbedder:
    ...
    def __init__(self, theta=256.0, axes_dims=None, axes_lens=None):
        ...
        self.freqs_cis = []
        for d, e in zip(axes_dims, axes_lens, strict=False):
...
class ZImageTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.all_x_embedder = nn.Linear(embed_dim, dim, bias=True)
    ...
    def __call__(self, x, timestep, sigmas, cap_feats,
        ...
        # ControlNet refiner stage — runs on the PRE-noise_refiner x_emb: embed
        # the 33-ch control context (patchified like the image → 132), run the 2
        # control_noise_refiner blocks → refiner_hints
        # (added into the main noise_refiner below) + the processed control_context.
        refiner_hints = layer_hints = mapping = control_context = None
        if controlnet is not None and control_image is not None:
            c_img, _, _, _, _, c_pad, _ = self._patchify(control_image, cap_feats)
            c_emb = controlnet.control_x_embedder(c_img)
        ...
        for j, layer in enumerate(self.noise_refiner):
            x_emb = layer(x_emb, x_mask, x_freqs, t_emb)
            if refiner_hints is not None and j < len(refiner_hints):
                x_emb = x_emb + refiner_hints[j] * control_scale
        ...
        for i, layer in enumerate(self.layers):
            ...
            if layer_hints is not None and i in mapping:
                unified = unified + layer_hints[mapping[i]] * control_scale
...
class VAE:
    ...
    def encode_tiled(
        ...
        if tile_size_px >= H and tile_size_px >= W:
    ...
    for key, val in list(d.items()):
        if isinstance(val, dict):
            ...
            if val and all(k.isdigit() for k in val):
                indices = sorted((int(k), k) for k in val)
...
class ImageGenEngine(ActiveRequestMixin):
    ...
    def load(self) -> None:
        ...
        # Lance/Wan-style flat layouts (model.safetensors + vae.safetensors +
        # vit.safetensors at the root, no transformer/ text_encoder/ vae/
        # subdirectories) cannot be loaded by the Z-Image pipeline. These models
        # were classified IMAGE_GEN by the model_manager via the
        # conversion_report.json "lance" variant heuristic, but no Lance image
        # generation forward/decode path exists in this engine yet. Treat the
        # load as a deferred no-op (matches VideoEngine's lazy start) so the
        # engine becomes registered without an exception; downstream generate()
        # calls will surface a clearer NotImplementedError than a misleading
        # FileNotFoundError on transformer/config.json.
        if not (model_path / "transformer").is_dir():
            logger.warning(
                "ImageGenEngine: model %s lacks transformer/ subdirectory "
                "(flat layout, e.g. Lance) — deferring Z-Image pipeline load; "
                "generate() will raise NotImplementedError until a matching "
                "pipeline is wired.",
                model_path.name,
            )
            self._tf_config = {}
            # Leave _transformer/_text_encoder/_vae/_tokenizer as None;
            # is_loaded stays False (transformer is None) but start() will
            # still mark _running=True so the engine slot is reserved.
            return
        ...
        # 4. Tokenizer
        # SECURITY: trust_remote_code=True executes any *.py shipped in the
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # load() failed partway — release any partially-loaded GPU
            # components so they don't leak, then re-raise.
            self._running = False
            await self.stop()
            raise
        self._running = True
        ...
        **kwargs,
        ...
        if self._transformer is None:
            ...
            raise NotImplementedError(
                f"Model {Path(self._model_path).name} has a flat (Lance/Wan-style) "
                "layout without a transformer/ subdirectory; the Z-Image pipeline "
                "cannot consume it. A dedicated Lance image pipeline is not yet "
                "wired into ImageGenEngine (deferred infra)."
            )

        # Validate dimensions. The real constraint is multiple-of-16,
        # NOT 8: the VAE downsamples by 8 AND the transformer patchifies the latent
        # by patch_size=2, so a %8-but-not-%16 size (e.g. 1080x1080) passed this
        # gate then crashed deep inside _patchify with an opaque MLX reshape error.
        # (The HTTP gateway already clamps to multiples of 64; this guards direct
        # engine calls.)
        ...
        **kwargs,
        ...
        if image is not None:
            return await self._generate_variation(
                ...
                # generate() swallowed cancel_event in **kwargs, so the router's
                # "honors cancel_event mid-diffusion" contract was false — img2img ran to
                # completion after a client disconnect. Thread it through to the denoise loop.
    ...
    async def _generate_variation(
        ...
        async def _watch_cancel():
            ...
            try:
                while cancel_event is not None and not _cancel.is_set():
    ...
    async def _run_cancellable(self, make_sync, cancel_event):
        """Run a diffusion sync closure in the MLX executor while honoring
        cancel_event. Covers controlnet/inpaint/depth-guided — without this, a client
        disconnect there ran all `n` diffusions to completion, head-of-line-blocking the
        serial executor. Mirror cancel_event into a thread-safe flag (executor work can't
        be killed by task.cancel), spawn a watcher to propagate a LATE disconnect, and
        pass the flag to the pipeline via make_sync.
        ...
        loop = asyncio.get_running_loop()
        _watcher = loop.create_task(_watch()) if cancel_event is not None else None
        try:
            return await loop.run_in_executor(self._executor, make_sync(_cancel))
        finally:
            if _watcher is not None:
                _watcher.cancel()
    ...
    async def generate_controlled_image(
        ...
        pose/edge map, PNG/JPEG bytes) via the Z-Image-Fun-Controlnet-Union model.
        control_scale ~0.65–1.0 (higher = stricter control)."""
        if self._transformer is None:
            raise RuntimeError("engine not started")
    ...
    async def generate_image_stream(
        ...
        **kwargs,
        ...
        # Inline <lora:NAME:WEIGHT> tags were silently DROPPED on the streaming
        # endpoint — _encode_prompt strips the tags but never loads the adapter, so a
        # streaming request produced a different (base-model) image than the identical
        # non-streaming request, which applies the LoRA via _apply_inline_loras. Parse +
        # apply the same way here, unloading in the finally.
        _clean_prompt, _lora_tags = _parse_lora_tags(prompt)
        ...
        def _stream_sync():
            ...
            try:
                _applied = self._apply_inline_loras(_lora_tags)
                # Tokenize + text-encode (with (word:weight) emphasis support)
                cap_feats = self._encode_prompt(_clean_prompt)

                latent_h = height // 8
                latent_w = width // 8
                latents = mx.random.normal(
                    shape=[16, 1, latent_h, latent_w],
                    # `seed or 42` treats seed=0 as falsy → 42, so the
                    # streaming path produced a DIFFERENT image than the non-streaming
                    # path (which uses `seed` directly) for seed=0. Honor 0.
                    key=mx.random.key(seed if seed is not None else 42),
                ).astype(mx.float16)
    ...
    def _resolve_sigmas(
        ...
        if self._diffusion_scheduler_type is not None:
            ...
            karras_sigmas = scheduler.sigmas  # list[float] — DDPM/Karras convention
            # DiffusionScheduler emits Karras/DDPM sigmas sqrt((1-abar)/abar) in
            # range ~[0.03, 14.6] (signal scale 1, noise std = sigma). The denoise loop here
            # is FLOW-MATCHING (x_t = (1-s)*x0 + s*eps, s in [0,1], timestep = 1 - s). Feeding
            # the raw Karras sigmas made timestep = 1 - 14.6 = -13.6 (garbage embedding) and
            # s=14.6 wildly off-manifold for the Z-Image transformer → noise output. Convert
            # by matching the noise-to-signal ratio: flow-matching ratio is s/(1-s), Karras is
            # sigma_k, so s = sigma_k / (1 + sigma_k) maps [0,inf)->[0,1) preserving SNR and
            # the per-scheduler-type spacing. (Inverse of the diffusers flow-match resolution.)
            sigma_list = [sk / (sk + 1.0) for sk in karras_sigmas]
            ...
            logger.info(
                ...
                f"SNR-matched to flow-matching): {num_steps} steps, "
                f"sigmas range [{sigma_list[0]:.4f}, {sigma_list[-2]:.4f}]"
            )
            return sigmas
    ...
    def _encode_prompt(self, prompt: str):
        """Tokenize + text-encode a prompt into conditioning features, applying
        A1111/ComfyUI `(word:weight)` emphasis. Returns cap_feats[num_valid, dim].
        Shared by all pipelines so weighting works everywhere. No emphasis syntax →
        identical to the previous plain encode (weights all 1.0)."""
        tok = self._tokenizer
        # Strip inline <lora:...> tags so they never pollute the text
        # embedding. t2i applies+strips them before calling here; the img2img/inpaint/
        # controlnet/depth pipelines pass the RAW prompt, so without this the literal
        # tag text is tokenized as caption. (Actually APPLYING the LoRA weights on those
        # non-t2i paths remains a deferred enhancement — see wave666_audit4.)
        if "<lora:" in prompt:
            prompt, _ = _parse_lora_tags(prompt)
        parsed = _parse_prompt_attention(prompt)
        clean = "".join(seg for seg, _ in parsed)
        ...
        if has_w:
            # Generic middle-layer: align per-segment weights to tokens, then apply
            # DIRECTION-based weighting (magnitude-scaling is a no-op under Z-Image's
            # RMSNorm'd caption — see diffusion_features).
            offs = enc["offset_mapping"][0] if "offset_mapping" in enc else None
            weights = _build_token_weights(parsed, clean, formatted, offs, num_valid)
            cap_feats = _apply_prompt_weights(cap_feats, weights)
        mx.eval(cap_feats)
        return cap_feats
    ...
    def _run_pipeline(self, prompt, width, height, num_steps, seed,
        ...
        # 3. Prepare noise latents (MLX-native random)
        latent_h = height // 8
        latent_w = width // 8
        latents = mx.random.normal(
            shape=[16, 1, latent_h, latent_w],
    ...
    def _load_image_to_tensor(self, image_data: bytes,
        ...
        MUST match the generation size, else its latent patches misalign with the
    ...
    def _run_img2img_pipeline(
        ...
        # 2. Tokenize prompt
        # Tokenize + text-encode via the shared path ((word:weight) emphasis +
        # inline-LoRA-tag stripping handled centrally).
        cap_feats = self._encode_prompt(prompt)
        ...
        # denoise_strength<=0 means "keep source exactly" (the docstring above +
        # the /images router contract). The min(..., num_steps-1) clamp below would force
        # start_step=num_steps-1 → ONE denoise step that injects sigmas[num_steps-1] noise
        # (≈0.49 at the Turbo 4-step default) → the source is heavily re-rendered, not kept.
        # Pin start_step=num_steps so the loop is empty and the blend reads sigma_start=
        # sigmas[num_steps]=0.0 (the trailing sentinel) → latents = pure source.
        if denoise_strength <= 0.0:
            start_step = num_steps
        else:
            start_step = max(0, min(start_step, num_steps - 1))
            # int() floors start_step to 0 at low step counts (the Turbo default
            # num_steps=4 with the /images/edits default strength=0.8 → int(4*0.2)=0), so
            # sigma_start=sigmas[0]=1.0 (pure noise) and the SOURCE image is silently
            # DISCARDED — img2img/edits degrade to plain text-to-image. For any strength<1.0
            # keep start_step>=1 so the blend starts from a sigma<1.0 and the source
            # actually influences the result.
            if denoise_strength < 1.0 and num_steps >= 2:
    ...
    def _run_inpaint_pipeline(
        ...
        # 2. Create mask tensor (latent res for the denoise loop + pixel res for
        # the final paste-back composite).
        _mask_px = None
        if mask_data is not None:
            mask_tensor = self._load_mask(mask_data, latent_h, latent_w)
            _mask_px = self._load_mask(mask_data, height, width)
        elif mask_base64 is not None:
            mask_bytes = base64.b64decode(mask_base64)
            mask_tensor = self._load_mask(mask_bytes, latent_h, latent_w)
            _mask_px = self._load_mask(mask_bytes, height, width)
        else:
            # No mask → full image inpainting (entire canvas); no paste-back needed.
        ...
        # Compute sigma schedule first — the masked-region init must be blended at
        # the sigma that MATCHES the timestep the loop starts on (flow-matching),
        # not a raw strength interpolation.
        # (respects YUNSHU_DIFFUSION_SCHEDULER opt-in)
        sigmas = self._resolve_sigmas(num_steps, width, height)

        # Apply denoise strength via a start-step, mirroring _run_img2img_pipeline.
        # The OLD code blended the masked region by `denoise_strength` (a raw lerp)
        # but then ran the loop from t=0 with sigma_0≈1.0 — telling the transformer
        # the input was PURE noise when it was only strength-noised. The masked
        # region was fed at the wrong sigma every step and over-denoised. Now we
        # pick start_step = num_steps*(1-strength) and blend at sigmas[start_step].
        start_step = int(num_steps * (1.0 - denoise_strength))
        # denoise_strength<=0 = keep source exactly (sibling of the img2img fix).
        # The min(..., num_steps-1) clamp would otherwise force one noise-injecting step.
        # start_step=num_steps → empty loop + sigma_start=sigmas[num_steps]=0.0 → pure source.
        if denoise_strength <= 0.0:
            start_step = num_steps
        else:
            start_step = max(0, min(start_step, num_steps - 1))
            if denoise_strength < 1.0 and num_steps >= 2:
                # int() floors to 0 at low step counts → sigma_start=1.0 (pure noise)
                # would silently discard the source. Keep >=1 so the source influences.
                start_step = max(1, start_step)
        ...
        if start_step == 0 and denoise_strength >= 1.0:
            ...
            # Flow-matching interpolation: x = (1 - sigma) * source + sigma * noise
            masked_init = (1.0 - sigma_start) * known_latents_4d + sigma_start * noise
        # Initialize the KEPT region noised to sigma_start TOO (not the clean
        # sigma≈0 encoding), so the FIRST denoise step (t=start_step) feeds the transformer an
        # ON-MANIFOLD kept context for the high-sigma timestep it is told. The old init left
        # the kept region clean while step 0 ran at sigma_start → off-distribution → boundary
        # seams at the worst (highest-sigma) step. The in-loop RePaint composite below already
        # does this for every step AFTER the first (re-noising to sigma_{t+1}); this extends it
        # to step 0. (At denoise_strength>=1 / start_step=0, sigma_start = sigmas[0] ≈ 0.94.)
        known_init = (1.0 - sigma_start) * known_latents_4d + sigma_start * noise
        latents = (1 - mask_4d) * known_init + mask_4d * masked_init
        ...
        # Pixel-space paste-back of the SOURCE in the KEPT (unmasked) region. The
        # kept latents end at the clean VAE encoding (final sigma_next=0 from the trailing
        # schedule 0), so decode(latents) returns decode(encode(source)) there — a LOSSY VAE
        # round-trip that softens / colour-shifts / detail-degrades the WHOLE untouched area,
        # breaking the "unmasked region preserved from the original" contract (the router and
        # _load_mask docstrings). Composite the decoded result with the source in PIXEL space
        # (both in VAE [-1,1], shape (1,3,H,W); mask 1=inpaint/0=keep) so only the masked
        # region uses the freshly generated pixels and the kept region stays faithful.
        if _mask_px is not None:
            image = self._composite_inpaint(image, image_tensor, _mask_px)
        mx.eval(image)
    ...
    def _composite_inpaint(generated: mx.array, source: mx.array, mask_px: mx.array) -> mx.array:
        ...
        generated/source: (1,3,H,W) in the same value space. The mask broadcasts across the
        3 channels. Returns mask*generated + (1-mask)*source so the kept region is byte-exact
        to the source rather than a lossy decode(encode(source))."""
        m = mask_px.astype(generated.dtype)
        return m * generated + (1.0 - m) * source.astype(generated.dtype)
    ...
    def _load_mask(self, mask_data: bytes, latent_h: int, latent_w: int) -> mx.array:
        ...
        # stored RGB happened to be non-black (some encoders fill white under full alpha=0) was
        # wrongly read as white → inpaint. This endpoint's documented convention is
        # white=inpaint / black=keep, so a transparent pixel must map to KEEP: flatten over
        # BLACK first so any transparent pixel becomes black regardless of its stored RGB. An
    ...
    def _to_png(image: mx.array) -> bytes:
        ...
        # Denormalize: VAE outputs [-1, 1] → [0, 1]
        arr = np.clip(arr / 2.0 + 0.5, 0, 1)
        # ROUND before the uint8 cast. A bare .astype truncates toward zero,
        # biasing every channel down by up to ~1/255 vs the reference
        # (images * 255).round().astype("uint8") — a systematic parity drift on every
        # generated image (all endpoints share _to_png). Values are clipped to [0,1] so
        # round() cannot overflow 255.
    ...
    def _run_depth_guided_pipeline(
        ...
        # Removed a dead `depth_latents *= depth_strength` here — the
        # later per-channel standardization (depth_4d - mean)/std is scale-invariant
        # and fully erased it; depth_strength is correctly applied once as
        # depth_weight in the denoise loop. (Cosmetic; behavior unchanged.)
        ...
        # Standardize the depth latent to zero-mean/unit-std before
        # adding it as a bias. The raw VAE-encoded latent has a non-zero mean (it is
        # scaling_factor*(x-shift)), so adding it injects a persistent DC bias that pushes
        # the unit-normal flow-matching latent off-manifold and collapses the decoded image
        # toward black — the sibling ControlNetBlock.inject_condition documents and fixes
        # this exact failure (verified there: strength=1 → mean≈2.6/255). Removing the DC
        # term keeps it a gentle, fading, strength-scaled nudge.
        _d_mean = depth_4d.mean()
        _d_std = mx.maximum(depth_4d.std(), mx.array(1e-3, dtype=depth_4d.dtype))
        depth_4d = (depth_4d - _d_mean) / _d_std
    ...
    def load_lora_adapter(self, adapter_path: str, rank: int = 8, scale: float = 20.0) -> bool:
        ...
        if self._transformer is None:
            ...
            return False
        # Auto-route: a standalone .safetensors file is a diffusion/ComfyUI-format
    ...
    def _load_lora_adapter_locked(self, adapter_path: str, rank: int = 8, scale: float = 20.0) -> bool:
        ...
        # Guard: prevent double-wrap (LoRALinear wrapping LoRALinear).
        # If _original_modules is non-empty, a LoRA adapter is already loaded
        # and must be unloaded first. Without this check, calling
        # load_lora_adapter twice wraps LoRALinear inside another LoRALinear,
        # producing garbage output.
        if self._original_modules:
            logger.error(
                "LoRA adapter already loaded — unload first to prevent double-wrap"
            )
            return False
        ...
        try:
            ...
            for name, module in self._transformer.named_modules():
                ...
                if applied >= num_layers:
                    break
                # Apply to attention projections (standard LoRA targets).
                # The Z-Image DiT names them to_q/to_k/to_v/to_out (see
                # _DiTAttention) — the old set {q_proj,v_proj,qkv} matched NOTHING
                # in self._transformer (those live only in the separate text
                ...
                if any(k in name for k in (
                    ...
                    # out_features attributes (it exposes .weight of shape [out,in]),
                    # so the old LoRALinear(module.in_features, module.out_features,
                    # ...) raised AttributeError on every module → the whole PEFT
                    # load crashed→return False (masked by the FakeTransformer test
                    # only asserting isinstance(result, bool)). from_base also handles
                    # quantized base layers (the Z-Image transformer is 4-bit).
                    lora_layer = LoRALinear.from_base(module, r=_rank, scale=_scale)
                    # Set the LoRA layer on the parent module. The DiT keeps its
                    # blocks in plain Python lists (self.layers/noise_refiner/
                    # context_refiner), so named_modules() yields names like
                    # "layers.0.attention.to_q" — the mid-path "0" is a LIST INDEX,
                    # not an attribute. A bare getattr(parent, "0") raises
                    # AttributeError('list' object has no attribute '0'), which made
                    # EVERY PEFT-directory image LoRA crash→return False on the real
                    # transformer (the FakeTransformer test uses flat top-level
                    # modules, hiding it). Walk index-aware, like _resolve_lora_module.
                    parts = name.rsplit(".", 1)
                    if len(parts) == 2:
                        parent = self._walk_module_parent(parts[0])
                        if parent is not None:
                            setattr(parent, parts[1], lora_layer)
                    applied += 1

            # Don't report success when nothing was wrapped — a
            # 0-layer "load" silently does nothing (wrong target names, or a
            # standalone-.safetensors adapter that belongs on load_diffusion_lora).
            if applied == 0:
                logger.warning(
                    "LoRA adapter %s matched 0 attention modules in the transformer "
                    "(expected to_q/to_v/to_out or q_proj/v_proj/qkv) — not applied. "
                    "For standalone ComfyUI .safetensors adapters use load_diffusion_lora.",
                    adapter_path,
                )
                return False
            ...
            # Restore original modules to prevent stale _original_modules from
            # blocking subsequent load_lora_adapter calls. Without this, a
            # partial failure (e.g. load_weights crash after some modules were
            # wrapped) leaves LoRALinear wrappers in place AND _original_modules
            # non-empty, so the next call hits the double-wrap guard and fails.
            try:
                for _name, _orig_mod in list(self._original_modules.items()):
                    _parts = _name.rsplit(".", 1)
                    if len(_parts) == 2:
                        _parent = self._walk_module_parent(_parts[0])
                        if _parent is not None:
                            setattr(_parent, _parts[1], _orig_mod)
                self._original_modules.clear()
            except Exception:
                logger.debug("LoRA partial failure restore failed", exc_info=True)
            return False
    ...
    def unload_lora_adapter(self) -> bool:
        ...
        try:
            for name, orig_module in self._original_modules.items():
                parts = name.rsplit(".", 1)
                if len(parts) == 2:
                    # Index-aware walk (mid-path list indices, e.g. "layers.0"); see
                    # the matching note in _load_lora_adapter_locked.
                    parent = self._walk_module_parent(parts[0])
                    if parent is not None:
    ...
    def _walk_module_parent(self, dotted: str):
        """Walk a dotted module path (mid-path list indices like 'layers.0' allowed)
        from self._transformer and return the resolved object, or None if any hop
        fails. Used by the PEFT-directory LoRA load/unload to set/restore a wrapped
        submodule whose parent lives inside a Python list (the DiT blocks)."""
        obj = self._transformer
    ...
    # ── Diffusion / ComfyUI-format LoRA (standalone .safetensors) ──
    def _resolve_lora_module(self, path: str):
        """Walk a dotted module path (with list indices, e.g. 'layers.0.attention.to_k')
        from self._transformer. Returns (target, parent, last_attr) or (None,None,None).
        The final attr is always a named attribute (setattr-able); list indices only
        occur mid-path."""
        parts = path.split(".")
        obj = self._transformer
        for p in parts[:-1]:
        ...
        return target, obj, last

    def _resolve_lora_file(self, name: str, search_dir: str = "models") -> str | None:
        """Resolve an inline-LoRA NAME to a .safetensors path (middle-layer)."""
        return _df_resolve_lora_file(name, search_dir)

    def _get_zimage_controlnet(self, path: str | None = None):
    ...
    def _build_control_context(self, control_tensor):
        """Build the 33-ch control context = concat[control_latent(16), mask(1)=0,
        inpaint_latent(16)=0] for pure-control (non-inpaint) use. control_tensor is a
        preprocessed control-map image tensor accepted by the VAE encoder."""
        control_latent = self._vae.encode_deterministic(control_tensor)
        # Normalize to [C, F=1, h, w] (VAE may return [C,h,w] or [1,C,h,w]).
        if control_latent.ndim == 3:
            control_latent = control_latent[:, None, :, :]
        elif control_latent.ndim == 4 and control_latent.shape[0] == 1:
            control_latent = control_latent.transpose(1, 0, 2, 3)
    ...
    def _apply_inline_loras(self, tags) -> bool:
        ...
        any loaded (so the caller knows to unload after generation). Missing names
        are warned and skipped, not fatal."""
        applied = False
        for name, weight in tags:
            path = self._resolve_lora_file(name)
            if path is None:
                logger.warning("Inline LoRA '%s' not found on disk — ignored", name)
                continue
            if self.load_diffusion_lora(path, strength=weight, stack=applied):
                applied = True
    ...
    def load_diffusion_lora(self, lora_path: str, strength: float = 1.0,
        ...
        `stack=True` nests on top of an already-loaded LoRA (for multiple inline
        tags) instead of refusing."""
        if self._transformer is None:
            logger.error("Cannot load diffusion LoRA: transformer not loaded")
            return False
        if not self._lora_lock.acquire(timeout=30):
            logger.error("Cannot load diffusion LoRA: lock contention")
            return False
        try:
            return self._load_diffusion_lora_locked(lora_path, strength, stack)
        finally:
            self._lora_lock.release()

    @staticmethod
    def _zimage_lora_key_remap(k: str) -> str:
        """Z-Image adapter: ComfyUI LoRA key → our module path. Strip the
        `diffusion_model.` prefix and collapse Sequential indices that our model
        flattened into a single nn.Linear.

        CRITICAL: the attention output projection is named
        `attention.to_out.0` in the ComfyUI/Flux/Z-Image reference (a Sequential
        whose [0] is the Linear), but our _DiTAttention.to_out is a FLAT nn.Linear
        (see line 236, "Was [nn.Linear], now flat"). Without collapsing `.to_out.0`
        the remapped path stayed `...attention.to_out.0`, and _resolve_lora_module
        treated the trailing `0` as a list index → getattr(linear, "0") → None →
        the layer was SILENTLY skipped. That dropped the to_out adaptation on ALL
        30 DiT layers (12.5% of a real adapter's 240 modules) with no error, badly
    ...
    def unload_diffusion_lora(self) -> bool:
        """Restore the original Linear modules wrapped by load_diffusion_lora."""
        restore = getattr(self, "_diff_lora_restore", None)
        if not restore:
            return False
        if not self._lora_lock.acquire(timeout=30):
            return False
        try:
            # Reverse order so stacked (nested) wrappers unwrap back to the original.
            for parent, attr, orig in reversed(restore):
                setattr(parent, attr, orig)
            self._diff_lora_restore = []
            mx.eval(self._transformer.parameters())
            logger.info("Diffusion LoRA unloaded")
            return True
        finally:
            self._lora_lock.release()
