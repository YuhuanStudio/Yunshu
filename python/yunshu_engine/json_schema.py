...
def make_grammar_constraint(
    grammar_type: str,
    grammar: Any = None,
    base_sampler: Callable | None = None,
    tokenizer: Any = None,
) -> Any:
    """Create a constraint or constrained sampler based on grammar type.

    Unified dispatch function that supports:
    - "json_object" / "json_schema": JsonSchemaConstraint (JSON structured output)
    - "regex": RegexConstraint (regex pattern enforcement)
    - "choice": ChoiceConstraint (enumeration from a fixed list)

    When both base_sampler and tokenizer are provided, returns a ConstrainedSampler.
    Otherwise returns the raw constraint object (for use with ConstrainedSampler later).

    Args:
        grammar_type: One of "json_object", "json_schema", "regex", "choice"
        grammar: Type-specific grammar specification:
            - json_object: None
            - json_schema: dict (JSON Schema)
            - regex: str (regex pattern)
            - choice: list[str] (allowed choices)
        base_sampler: Optional underlying mlx-lm sampler function
        tokenizer: Optional tokenizer for token-level masking

    Returns:
        ConstrainedSampler if base_sampler and tokenizer provided,
        otherwise the raw constraint object.

    Raises:
        ValueError: If grammar_type is unknown or grammar is invalid.
    """
    from .grammar_constraint import RegexConstraint, ChoiceConstraint

    if grammar_type in ("json_object", "json_schema"):
        schema = grammar if grammar_type == "json_schema" and isinstance(grammar, dict) else None
        constraint = JsonSchemaConstraint(schema)
        ...
        raise ValueError(f"Unknown grammar_type: {grammar_type!r}")

    if base_sampler is not None and tokenizer is not None:
        return ConstrainedSampler(base_sampler, constraint, tokenizer)
    return constraint
