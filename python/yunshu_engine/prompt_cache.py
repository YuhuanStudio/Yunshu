...
_VOLATILE_HEADER_RE = re.compile(
    r"(?im)^(?:x-anthropic-billing-header|x-request-id|x-trace-id|x-session-id):[^\n]*(?:\n|$)"
)


def _canonicalize_system_content(text: str) -> str:
    """Strip known volatile non-semantic headers from system prompt text.

    Volatile lines (billing headers, trace IDs) change per-request but carry
    no semantic meaning. Stripping them before hashing prevents unnecessary
    cache misses when the semantic system prompt is identical.
    """
    return _VOLATILE_HEADER_RE.sub("", text)


def _canonicalize_messages(messages: list[dict]) -> list[dict]:
    """Return a copy of messages with volatile system-prompt content stripped."""
    result = []
    changed = False
    for msg in messages:
        if msg.get("role") == "system" and isinstance(msg.get("content"), str):
            clean = _canonicalize_system_content(msg["content"])
...
class PromptCacheManager:
    ...
    def lookup(self, messages_hash: str) -> CacheEntry | None:
    ...
    def _is_expired(self, entry: CacheEntry) -> bool:
        """Check if an entry has been IDLE longer than its TTL.

        Uses last_accessed, not created_at — the TTL is an idle timeout, not a
        fixed lifetime. A created_at check would expire a hot entry (e.g. a long system
        prompt reused every request) 1h after its FIRST store regardless of how often it
        was hit, forcing a needless re-prefill. last_accessed is refreshed on every
        lookup, so a frequently-reused prompt stays cached; a genuinely idle
        one still expires after ttl_seconds. LRU + max_entries still bound total retention.
        """
        idle = time.monotonic() - entry.last_accessed
        return idle > self._ttl_seconds
