...
def parse_qwen_tool_calls(text: str) -> list[ToolCallResult]:
    ...
    for i, m in enumerate(tag_re.finditer(text)):
        ...
        # produced a duplicate). The self-closing <tool_call/>{...} form has its
...
def parse_deepseek_tool_calls(text: str) -> list[ToolCallResult]:
    ...
    DeepSeek uses fullwidth chars in markers: ｜ (U+FF5C) for pipes and ▁
    (U+2581) for underscores. Two real on-the-wire formats exist (verified
    against reference/llama.cpp/tests/test-chat.cpp):

    * **DeepSeek-V3** (current): name sits between the markers, args follow ``sep``
      directly with no fence::

        <｜tool▁call▁begin｜>get_time<｜tool▁sep｜>{"city":"Tokyo"}<｜tool▁call▁end｜>

    * **DeepSeek-R1** (older): a literal ``function`` precedes ``sep``, the name
      follows it, and args are in a ```` ```json ```` fence::

        <｜tool▁call▁begin｜>function<｜tool▁sep｜>get_time\n```json\n{...}\n```

    The old regex (``tool▁callBegin｜function｜tool▁sep｜…```json``) matched NEITHER
    (missing ▁ in "call▁begin", a "function" literal that V3 doesn't emit, and a
    mandatory ```` ```json ```` fence) so every DeepSeek tool call was silently dropped.
    """
    results = []
...
def parse_glm_tool_calls(text: str) -> list[ToolCallResult]:
    """Parse GLM-style tool calls.

    Two formats (verified vs reference/llama.cpp/tests/test-chat.cpp):

    * older: ``<|tool_call_block_begin|>name\n```json\n{...}\n``` ``
    * GLM-4.6 / 4.7: ``<tool_call>name<arg_key>k</arg_key><arg_value>v</arg_value>…</tool_call>``
      — args are key/value PAIRS, not a JSON object; the old parser handled only
    ...
    for i, m in enumerate(block_re.finditer(text)):
        ...
        for k, v in _GLM_ARG_RE.findall(body):
            k = k.strip()
            v = v.strip()
            # Values are emitted raw; recover JSON scalars (numbers/bools/objects),
...
def parse_generic_tool_calls(text: str) -> list[ToolCallResult]:
    ...
    # Collect the enclosing-object range for EACH "name" occurrence, then keep only
    # the OUTERMOST objects. A "name" key INSIDE another tool-call object (e.g. a
    # `name` field inside the arguments dict — `{"name":"f","arguments":{"name":...}}`)
    # would otherwise be walked back to the inner brace and emitted as a PHANTOM tool
    # call. Filtering nested ranges is order-independent (works whether the real
    # "name" precedes or follows "arguments").
    candidates = []  # (brace_start, brace_end, block)
    for match in name_re.finditer(text):
        ...
        if block is None:
            continue
        candidates.append((brace_start, brace_start + len(block), block))
