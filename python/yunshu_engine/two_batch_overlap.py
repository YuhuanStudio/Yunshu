...
"""Yunshu Two-Batch Overlap (TBO) Scheduler.
...
class TwoBatchOverlapScheduler:
    ...
    def _should_use_sequential(self, running_count: int) -> bool:
        ...
        return self._metrics.low_efficiency_streak >= self._config.fallback_window
    ...
    def _count_running_requests(self, scheduler: Any) -> int:
        ...
        if batch_gen is not None and hasattr(batch_gen, "active_count"):
    ...
    def should_enable_tbo(self) -> bool:
        ...
        return not (len(recent) > 0 and multi_steps / len(recent) < 0.3)
