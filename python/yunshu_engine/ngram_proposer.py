...
1. **LPS mode**: KMP-based O(n) matching via LPS array
...
def _find_longest_ngram_and_propose(
    ...
    # Cap the scan window to avoid O(n²) on very long contexts.
    # Only search the last _SCAN_WINDOW tokens — older matches are
    # unlikely to be relevant and the hashpool mode handles long-range
    # matches with O(1) lookup.
    _SCAN_WINDOW = 4096
...
class NgramProposer:
    ...
    Two modes:
    - "lps": KMP-based O(n) matching (good for short contexts)
    ...
    def __init__(self, config: NgramConfig) -> None:
        self.config = config
        self._hashpool: NgramHashPool | None = None
        self._lcg_pool: LCGHashPool | None = None
    ...
    def reset(self) -> None:
        ...
        Clears all pool entries and resets indexed length trackers.
        Without this, stale n-gram entries from a previous request
        contaminate proposal for the new request.
        """
        if self._hashpool is not None:
            self._hashpool.clear()
        if self._lcg_pool is not None:
