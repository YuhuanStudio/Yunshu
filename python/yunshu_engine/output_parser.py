...
Each model family has its own output format for
...
class QwenOutputParser(OutputParser):
    ...
    def parse(self, text: str) -> ParsedOutput:
        ...
        if think_matches:
            ...
            # CLOSING-ONLY: Qwen3/3.5 chat templates inject the opening <think>, so the
            # model output often has only </think>. Treat the text up to the first close
            # (when no open precedes it) as reasoning — else the CoT + </think> leak.
...
class LLamaOutputParser(OutputParser):
    ...
    _THINK_RE = re.compile(r"<think\s*/?\s*>(.*?)</think\s*/?\s*>", re.DOTALL)
    # the closing alternative was `<\|/python_tag\>` — missing the
    # second `|` (the real tag is `<|/python_tag|>`), so the close never matched
    # and the capture always ran to end-of-string, over-capturing any text after
    # a real closing tag. Fixed to match the actual tag.
