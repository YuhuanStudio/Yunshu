...
def detect_spec_heads(model_config: dict) -> SpecHeadInfo:
    ...
    using simple config inspection (no model loading needed).
    ...
    model_config.get("architectures", [])
...
def auto_configure_speculative(model_config: dict) -> SpecDecodingConfig:
    ...
    # Cap draft length to reasonable maximum
...
class SpeculativeDecoder:
    ...
    def generate_draft(
        ...
        for _ in range(K):
            ...
            # EOS guard: stop drafting if we hit an end-of-sequence token.
            # Without this, constraint.advance() would consume past EOS and
            # produce garbage constraint state for subsequent draft steps.
    ...
    def verify_draft(
        ...
        # The constraint state is at position K (all drafts advanced).
        # Roll back to pre-draft and re-advance through only accepted
        # tokens so the mask matches the actual acceptance position.
        if self.constraint is not None:
            try:
                # the constraint is at position K (all drafts advanced by
                # generate_draft). Roll back to pre-draft, then re-advance only the
                # accepted tokens so the bonus mask matches the real acceptance position.
                # JsonSchemaConstraint.rollback() is no-arg (internal checkpoint stack);
                # Regex/Choice/Lark.rollback(saved) REQUIRES a dict checkpoint we don't
                # hold in this method. Previously the no-arg call raised TypeError, was
                # suppressed, and the rollback was SKIPPED — then accepted_ids were
                # re-advanced ON TOP of the K-draft state, so the mask was computed from a
                # corrupted (over-long) buffer and could mask the correct bonus token.
                # Only re-mask when the rollback genuinely succeeded; for grammar
    ...
    def batch_verify_drafts(
        self,
        draft_results: list[tuple[DraftResult, mx.array, list]],
        temperature: float = 0.0,
    ) -> list[VerifyResult]:
        """Verify multiple draft sequences in a single batched forward pass.

        Concatenates all draft sequences into a single batch, runs one forward
        pass on the target model, then splits results per request. This is
        more efficient than calling verify_draft() N times when multiple
        requests have drafts ready simultaneously.

        Each request has its own target cache, so the forward pass is done
        per-request but with batched tensor construction to minimize Python
        overhead. The key optimization is that all log-probability
        computations are vectorized across draft positions.

        Args:
            draft_results: List of (DraftResult, last_token_ids, target_cache)
                tuples, one per request. Each ``last_token_ids`` is an
                ``mx.array`` of shape ``[1, 1]`` — the last generated token
                that serves as the alignment token for verification.
            temperature: Sampling temperature for bonus/correction tokens.

        Returns:
            List of VerifyResult, one per input request, in the same order.
        """
        ...
        for draft_result, last_token_ids, target_cache in draft_results:
            try:
                K = len(draft_result.token_ids)
                if K == 0:
                    results.append(
                ...
                # Build aligned input: [last_token, d0, d1, ..., dK-1]
                last_tok = last_token_ids[:, -1:]  # [1, 1]
                draft_tokens = mx.array(draft_result.token_ids).reshape(1, K)
                aligned_input = mx.concatenate([last_tok, draft_tokens], axis=1)

                # Single forward pass for this request's drafts
                _forward_ran = False
                ...
                # Vectorized acceptance ratio
                log_diff = mx.clip(target_lps_arr - draft_lps_arr, -50.0, 50.0)
                raw_ratios = mx.exp(log_diff)
                threshold = self.config.acceptance_threshold
                ratios = mx.minimum(mx.full(K, threshold), raw_ratios)

                # Generate uniform random numbers
                uniforms = mx.array([self.rng.random() for _ in range(K)])
                accepted_mask = uniforms < ratios

                # Sequential scan: find first rejection
                accepted_ids = []
                target_lps = []
                rejected_at = -1
                ...
                if self.constraint is not None:
                    try:
                        ...
                        # Grammar (Regex/Choice/Lark) rollback needs a checkpoint dict we
                        # don't hold here → no-arg call TypeErrors → skip the bonus mask
                ...
                # When forward ran: net gain is K (K+1 added - 1 trimmed), trim K.
                # When forward did NOT run: net loss is 1, trim 0 (already at N-1,
                # and we can't recover the lost entry).
        ...
        total_bonus = sum(1 for r in results if r.bonus_token_id >= 0)
        self._stats["total_draft_tokens"] += total_draft
        self._stats["total_accepted_tokens"] += total_accepted
        self._stats["total_bonus_tokens"] += total_bonus
        self._stats["total_steps"] += len(draft_results)
    ...
    def _snapshot_cache(cache: list) -> list:
        ...
        RotatingKVCache: save offset + _idx
        ChunkedKVCache: save offset + start_position
        CacheList: recursive snapshot of wrapped caches
        """
        snapshot = []
        for c in cache:
    ...
    def _restore_cache(cache: list, snapshot: list) -> None:
        ...
        For RotatingKVCache: restores offset + _idx.
        For ChunkedKVCache: restores offset + start_position.
