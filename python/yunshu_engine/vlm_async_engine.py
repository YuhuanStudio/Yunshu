...
"""Yunshu VLM Async Engine Core — concurrent VLM inference.
...
class VLMAsyncEngineCore:
    """Concurrent VLM inference engine.
    ...
    async def stop(self) -> None:
        ...
        for _req_id, task in list(self._tasks.items()):
            if not task.done():
                task.cancel()
    ...
    async def _process_request(self, state: _VLMRequestState) -> None:
        ...
        try:
            ...
            logger.error(f"VLM request {config.request_id} failed: {e}", exc_info=True)
            failed = True
