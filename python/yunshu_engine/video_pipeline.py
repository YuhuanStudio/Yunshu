...
class VideoGenRequest:
    ...
    prompt: str = ""
    negative_prompt: str = ""
    image: Optional[mx.array] = None  # Optional source image for I2V (H, W, C)
...
class VideoGenResult:
    ...
    latents: Optional[mx.array] = None  # Final latents (1, C, T, H_lat, W_lat)
    width: int = 0
    height: int = 0
    num_frames: int = 0
    fps: int = 16
...
class WanVideoPipeline(VideoPipeline):
    ...
    def __init__(
        ...
        self._vae_decoder: Optional[VideoVAEDecoder] = None
    ...
    def generate_frames(self, request: VideoGenRequest) -> VideoGenResult:
        ...
        if self._model is not None:
            ...
            logger.warning(
                "No transformer model weights loaded. Generating noise-based "
        ...
        return VideoGenResult(
            ...
            fps=request.fps,
            duration_s=elapsed,
            method=f"wan_native_{request.scheduler}",
            seed_used=seed,
        )
    ...
    def generate_from_image(
        ...
        return VideoGenResult(
            ...
            fps=request.fps,
            duration_s=elapsed,
            method=f"wan_i2v_native_{self._scheduler_type}",
            seed_used=seed,
