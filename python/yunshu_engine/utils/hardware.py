...
single source of truth for chip identification,
...
def get_memory_bandwidth_gb() -> float | None:
    ...
    # Substring scan must check the longest key first — otherwise "M3" in
    # "Apple M3 Max" wins before "M3 Max" gets a chance (returns 100 instead
    # of 400 for M3 Max).
    for key in sorted(_MEMORY_BANDWIDTH.keys(), key=len, reverse=True):
        if key in chip:
            return _MEMORY_BANDWIDTH[key]
...
def get_ane_available() -> bool | None:
...
def compute_adaptive_defaults(hw: HardwareInfo | None = None) -> dict:
    ...
    auto-tune batch size, KV cache limits, prefill chunks,
