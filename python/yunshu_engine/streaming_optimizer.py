    ...
    *previous* token's completed stage3 result.  The current token's
    ...
    async def next_token(
        ...
        if self.config.enable_overlap:
            ...
            #
            # To prevent an orphaned task on cancellation between
            # the create_task() and the subsequent await, we publish the new
            # task to self._prev_stage3_task BEFORE awaiting the previous one.
            # The previous task is captured in a local first so it can still
            # be awaited.  If an exception/cancellation interrupts the await,
            # the new stage3 task is reachable via self._prev_stage3_task and
            # can be cancelled in finish()/cleanup.
            prev_task = self._prev_stage3_task
            ...
            if prev_task is not None:
                ...
                try:
                    self._completed_token = await prev_task
                except Exception:
                ...
                # Already awaited — clear the published reference so finish()
                # won't try to cancel a completed task.
                self._prev_stage3_task = None
