...
type Tab = "models" | "keys" | "tenants" | "audit" | "logs" | "cache" | "hardware";
...
function TenantManager() {
  const [tenants, setTenants] = useState<Array<{ id: string; name: string; tier: string; active: boolean; quota_rpm: number }>>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 503) {
        ...
        setError(d.detail ?? "Tenant management not enabled (set YUNSHU_TENANT_PATH)");
        setTenants([]);
        return;
      }
      const d = await r.json();
  ...
  const create = async () => {
    setNewKey(null);
    setError(null);
    const body: Record<string, unknown> = { name: name.trim(), tier };
    if (rpm) body.requests_per_minute = parseInt(rpm, 10);
    const r = await fetch("/api/v1/admin/tenants", {
      method: "POST",
  ...
  const deactivate = async (id: string) => {
    if (!confirm(`Deactivate tenant ${id}?`)) return;
    await fetch(`/api/v1/admin/tenants/${id}/deactivate`, { method: "POST" });
    load();
  };
      ...
      <div className="border border-[var(--color-border)] rounded-lg p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="px-3 py-1.5 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-md text-sm"
          />
          <select
            value={tier}
          ...
          <input
            placeholder="Custom RPM (optional)"
            value={rpm}
            onChange={(e) => setRpm(e.target.value)}
            type="number"
            className="px-3 py-1.5 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-md text-sm w-44"
          />
          <button
            onClick={create}
            disabled={!name.trim()}
            className="px-4 py-1.5 bg-[var(--color-accent)] text-white rounded-md text-sm disabled:opacity-40"
          >
            Create
          </button>
        </div>
        {newKey && (
          <div className="text-sm bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded p-2">
            <div className="font-semibold">API key (save it — cannot recover):</div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--color-bg-tertiary)]">
            <tr>
              ...
              <th className="text-left px-3 py-2">RPM</th>
              <th className="text-left px-3 py-2">Active</th>
              <th className="text-right px-3 py-2">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading && <tr><td colSpan={6} className="px-3 py-4 text-center text-[var(--color-text-secondary)]"><Loader2 className="w-4 h-4 inline animate-spin" /> Loading...</td></tr>}
            {!loading && tenants.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-[var(--color-text-secondary)]">No tenants</td></tr>
            )}
            {tenants.map((t) => (
              <tr key={t.id} className="border-t border-[var(--color-border)]">
                ...
                <td className="px-3 py-2">{t.quota_rpm}</td>
                <td className="px-3 py-2">{t.active ? "✓" : "✗"}</td>
                <td className="px-3 py-2 text-right space-x-2">
                  {t.active && (
                    <button onClick={() => deactivate(t.id)} className="text-xs text-yellow-600 hover:underline">deactivate</button>
                  )}
...
function AuditLogViewer() {
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/v1/admin/audit-log?${params}`);
    if (r.ok) {
      const d = await r.json();
      setEvents(d.events || []);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <input
          placeholder="Filter by op (e.g. tenant_create)"
          value={opFilter}
          onChange={(e) => setOpFilter(e.target.value)}
          className="px-3 py-1.5 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-md text-sm flex-1"
        />
        <input
          type="number"
          value={limit}
          onChange={(e) => setLimit(parseInt(e.target.value, 10) || 50)}
          className="px-3 py-1.5 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-md text-sm w-24"
        />
        <button onClick={load} className="px-3 py-1.5 bg-[var(--color-accent)] text-white rounded-md text-sm">
      ...
      <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading && <tr><td colSpan={5} className="px-3 py-4 text-center"><Loader2 className="w-4 h-4 inline animate-spin" /></td></tr>}
            {!loading && events.length === 0 && (
              <tr><td colSpan={5} className="px-3 py-4 text-center text-[var(--color-text-secondary)]">No audit events</td></tr>
            )}
            {events.map((e, i) => (
              <tr key={i} className="border-t border-[var(--color-border)]">
                <td className="px-3 py-1.5 text-xs text-[var(--color-text-secondary)]">{(e.ts || "").slice(0, 19)}</td>
                <td className="px-3 py-1.5 font-mono text-xs">{e.op}</td>
                <td className="px-3 py-1.5 text-xs">{e.actor}</td>
                <td className="px-3 py-1.5 font-mono text-xs">{e.resource}</td>
                <td className={`px-3 py-1.5 text-xs ${e.result === "success" ? "text-green-600" : "text-red-600"}`}>{e.result}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ModelSettings() {
  ...
  useEffect(() => {
    // Wave 432: cancel in-flight fan-out on models change to avoid stale
    // settings writes (a slow earlier fetch could otherwise overwrite a
    // newer one). AbortController guarantees the .then() short-circuits.
    const ctrl = new AbortController();
    models.forEach((m) => {
      fetch(`/api/v1/admin/models/${encodeURIComponent(m.id)}/settings`, { signal: ctrl.signal })
        .then((r) => r.json())
...
function SettingInput({ label, value, onChange, type = "number", step = 1 }: {
  label: string; value: unknown; onChange: (v: number) => void; type?: "number"; step?: number;
}) {
  // Wave 432: tighten to number-only — non-number branch silently passed 0 (real bug),
  // and all existing callers use type="number" anyway. Default is "number"; type is
  // narrowed at the type level so future callers can't request text/email/etc.
  return (
    <div>
